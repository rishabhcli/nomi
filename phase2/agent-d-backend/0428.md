# [D-0428] MemoryDynamics: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `MemoryDynamics` using **citation verifier false-positive elimination** (seed `c50b26561d0f`).

Deliverables:
- Identify ≥3 concrete failure modes in current `MemoryDynamics` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `MemoryDynamics`.
- If `MemoryDynamics` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/MemoryDynamics.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0428.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0429 (or finish at 1000)
