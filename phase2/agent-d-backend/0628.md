# [D-0628] Ingestion: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `Ingestion` using **citation verifier false-positive elimination** (seed `0da7c863d1ec`).

Deliverables:
- Identify ≥3 concrete failure modes in current `Ingestion` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `Ingestion`.
- If `Ingestion` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/Ingestion.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0628.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0629 (or finish at 1000)
