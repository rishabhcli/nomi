# [D-0548] TimelineBuilder: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `TimelineBuilder` using **citation verifier false-positive elimination** (seed `88e92451e938`).

Deliverables:
- Identify ≥3 concrete failure modes in current `TimelineBuilder` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `TimelineBuilder`.
- If `TimelineBuilder` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/TimelineBuilder.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0548.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0549 (or finish at 1000)
