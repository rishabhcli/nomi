...
As a **staff backend engineer**, harden `TimeWindow` using **citation verifier false-positive elimination** (seed `15aae7bc0528`).

Deliverables:
- Identify ≥3 concrete failure modes in current `TimeWindow` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `TimeWindow`.
- If `TimeWindow` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/TimeWindow.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0088.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0089 (or finish at 1000)
