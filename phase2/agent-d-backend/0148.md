...
As a **staff backend engineer**, harden `LocalExtractor` using **citation verifier false-positive elimination** (seed `527521255b54`).

Deliverables:
- Identify ≥3 concrete failure modes in current `LocalExtractor` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `LocalExtractor`.
- If `LocalExtractor` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/LocalExtractor.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0148.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0149 (or finish at 1000)
