...
As a **staff backend engineer**, harden `PersonalRanker` using **citation verifier false-positive elimination** (seed `8bb66b0c2efd`).

Deliverables:
- Identify ≥3 concrete failure modes in current `PersonalRanker` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `PersonalRanker`.
- If `PersonalRanker` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/PersonalRanker.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0188.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0189 (or finish at 1000)
