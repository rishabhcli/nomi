# [D-0511] QueryService: agentic grep deadlock prevention
...
As a **staff backend engineer**, harden `QueryService` using **agentic grep deadlock prevention** (seed `9e9eb4cd1432`).
...
- [ ] Evidence: `phase2/evidence/D-0511.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0512 (or finish at 1000)
