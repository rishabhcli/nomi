# [D-0268] ContextAssembler: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `ContextAssembler` using **citation verifier false-positive elimination** (seed `80cb650e0e5a`).

Deliverables:
- Identify ≥3 concrete failure modes in current `ContextAssembler` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `ContextAssembler`.
- If `ContextAssembler` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/ContextAssembler.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0268.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0269 (or finish at 1000)
