...
As a **staff backend engineer**, harden `Consolidation` using **concurrency stress under WorkScheduler** (seed `dcf798c5a27f`).

Deliverables:
- Identify ≥3 concrete failure modes in current `Consolidation` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `Consolidation`.
- If `Consolidation` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/Consolidation.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0022.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0023 (or finish at 1000)
