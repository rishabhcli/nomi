# [D-0868] QueryService: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `QueryService` using **citation verifier false-positive elimination** (seed `45e70ffa9b79`).

Deliverables:
- Identify ≥3 concrete failure modes in current `QueryService` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `QueryService`.
- If `QueryService` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/QueryService.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0868.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0869 (or finish at 1000)
