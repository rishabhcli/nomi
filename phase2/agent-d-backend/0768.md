# [D-0768] RouterEscalator: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `RouterEscalator` using **citation verifier false-positive elimination** (seed `34e9321df97e`).

Deliverables:
- Identify ≥3 concrete failure modes in current `RouterEscalator` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `RouterEscalator`.
- If `RouterEscalator` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/RouterEscalator.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0768.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0769 (or finish at 1000)
