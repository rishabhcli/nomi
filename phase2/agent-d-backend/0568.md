# [D-0568] CitationVerifier: citation verifier false-positive elimination

| Field | Value |
|-------|-------|
| **Queue** | 568 / 1000 |
| **Agent** | D — Backend / Query Brain |
| **Phase** | Observability Contracts — Events, logs, traces, debuggability |
| **Priority** | P2 |
| **Seed** | `72800024119f` |
...
As a **staff backend engineer**, harden `CitationVerifier` using **citation verifier false-positive elimination** (seed `72800024119f`).

Deliverables:
- Identify ≥3 concrete failure modes in current `CitationVerifier` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `CitationVerifier`.
- If `CitationVerifier` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/CitationVerifier.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0568.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0569 (or finish at 1000)
