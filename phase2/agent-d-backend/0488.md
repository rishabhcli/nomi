...
As a **staff backend engineer**, harden `QueryRewriter` using **citation verifier false-positive elimination** (seed `1cba84c0ed58`).

Deliverables:
- Identify ≥3 concrete failure modes in current `QueryRewriter` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `QueryRewriter`.
- If `QueryRewriter` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/QueryRewriter.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0488.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0489 (or finish at 1000)
