...
As a **staff backend engineer**, harden `OllamaClient` using **citation verifier false-positive elimination** (seed `53ca74b28e8e`).

Deliverables:
- Identify ≥3 concrete failure modes in current `OllamaClient` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `OllamaClient`.
- If `OllamaClient` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/OllamaClient.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0168.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0169 (or finish at 1000)
