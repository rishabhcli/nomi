...
As a **staff backend engineer**, harden `KeywordBackstop` using **concurrency stress under WorkScheduler** (seed `b57899c9babc`).

Deliverables:
- Identify ≥3 concrete failure modes in current `KeywordBackstop` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `KeywordBackstop`.
- If `KeywordBackstop` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/KeywordBackstop.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0062.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0063 (or finish at 1000)
