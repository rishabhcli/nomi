...
As a **staff backend engineer**, harden `EngineIntegration` using **citation verifier false-positive elimination** (seed `6e79132442f4`).

Deliverables:
- Identify ≥3 concrete failure modes in current `EngineIntegration` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `EngineIntegration`.
- If `EngineIntegration` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/EngineIntegration.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0108.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0109 (or finish at 1000)
