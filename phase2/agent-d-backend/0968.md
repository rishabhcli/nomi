# [D-0968] Highlight: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `Highlight` using **citation verifier false-positive elimination** (seed `9a186d12fced`).

Deliverables:
- Identify ≥3 concrete failure modes in current `Highlight` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `Highlight`.
- If `Highlight` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/Highlight.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0968.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0969 (or finish at 1000)
