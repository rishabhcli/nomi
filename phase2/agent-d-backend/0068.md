...
As a **staff backend engineer**, harden `IngestGate` using **citation verifier false-positive elimination** (seed `af46684b2afc`).

Deliverables:
- Identify ≥3 concrete failure modes in current `IngestGate` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `IngestGate`.
- If `IngestGate` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/IngestGate.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0068.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0069 (or finish at 1000)
