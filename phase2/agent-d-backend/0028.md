...
As a **staff backend engineer**, harden `NotchReducer` using **citation verifier false-positive elimination** (seed `8c526c68a54e`).

Deliverables:
- Identify ≥3 concrete failure modes in current `NotchReducer` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `NotchReducer`.
- If `NotchReducer` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/NotchReducer.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0028.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0029 (or finish at 1000)
