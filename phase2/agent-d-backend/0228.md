...
As a **staff backend engineer**, harden `Inspector` using **citation verifier false-positive elimination** (seed `be6d8c2ccaa5`).

Deliverables:
- Identify ≥3 concrete failure modes in current `Inspector` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `Inspector`.
- If `Inspector` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/Inspector.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0228.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0229 (or finish at 1000)
