...
As a **staff backend engineer**, harden `AnswerCache` using **citation verifier false-positive elimination** (seed `752f33c29467`).

Deliverables:
- Identify ≥3 concrete failure modes in current `AnswerCache` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `AnswerCache`.
- If `AnswerCache` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/AnswerCache.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0288.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0289 (or finish at 1000)
