...
| **Phase** | Core Path Hardening — Query lifecycle, retrieval, citations, memory |
| **Priority** | P1 |
| **Seed** | `b4b478a4ca20` |
...
As a **staff backend engineer**, harden `EvidenceGathering` using **citation verifier false-positive elimination** (seed `b4b478a4ca20`).

Deliverables:
- Identify ≥3 concrete failure modes in current `EvidenceGathering` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `EvidenceGathering`.
- If `EvidenceGathering` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/EvidenceGathering.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0208.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0209 (or finish at 1000)
