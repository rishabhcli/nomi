...
As a **staff backend engineer**, harden `Provenance` using **concurrency stress under WorkScheduler** (seed `5c2083671b10`).

Deliverables:
- Identify ≥3 concrete failure modes in current `Provenance` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `Provenance`.
- If `Provenance` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/Provenance.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0042.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0043 (or finish at 1000)
