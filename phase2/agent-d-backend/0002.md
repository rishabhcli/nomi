# [D-0002] Router: concurrency stress under WorkScheduler
...
As a **staff backend engineer**, harden `Router` using **concurrency stress under WorkScheduler** (seed `a27199799d0b`).

Deliverables:
- Identify ≥3 concrete failure modes in current `Router` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `Router`.
- If `Router` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/Router.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0002.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0003 (or finish at 1000)
