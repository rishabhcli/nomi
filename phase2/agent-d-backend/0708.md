# [D-0708] MediaCompanion: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `MediaCompanion` using **citation verifier false-positive elimination** (seed `eef3ef76bfa2`).

Deliverables:
- Identify ≥3 concrete failure modes in current `MediaCompanion` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `MediaCompanion`.
- If `MediaCompanion` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/MediaCompanion.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0708.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0709 (or finish at 1000)
