# [D-0988] ContentHash: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `ContentHash` using **citation verifier false-positive elimination** (seed `d5bbe8f62d24`).

Deliverables:
- Identify ≥3 concrete failure modes in current `ContentHash` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `ContentHash`.
- If `ContentHash` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/ContentHash.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0988.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0989 (or finish at 1000)
