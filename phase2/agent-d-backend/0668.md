# [D-0668] EngineClient: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `EngineClient` using **citation verifier false-positive elimination** (seed `12feef84259c`).

Deliverables:
- Identify ≥3 concrete failure modes in current `EngineClient` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `EngineClient`.
- If `EngineClient` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/EngineClient.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0668.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0669 (or finish at 1000)
