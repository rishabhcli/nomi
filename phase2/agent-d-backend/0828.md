# [D-0828] LLMHopPlanner: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `LLMHopPlanner` using **citation verifier false-positive elimination** (seed `c1746e7a41c9`).

Deliverables:
- Identify ≥3 concrete failure modes in current `LLMHopPlanner` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `LLMHopPlanner`.
- If `LLMHopPlanner` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/LLMHopPlanner.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0828.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0829 (or finish at 1000)
