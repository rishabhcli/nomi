# [D-0588] WorkScheduler: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `WorkScheduler` using **citation verifier false-positive elimination** (seed `4e2112384eea`).

Deliverables:
- Identify ≥3 concrete failure modes in current `WorkScheduler` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `WorkScheduler`.
- If `WorkScheduler` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/WorkScheduler.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0588.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0589 (or finish at 1000)
