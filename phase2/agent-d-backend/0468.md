...
As a **staff backend engineer**, harden `CharSpan` using **citation verifier false-positive elimination** (seed `f251672ee4bd`).

Deliverables:
- Identify ≥3 concrete failure modes in current `CharSpan` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `CharSpan`.
- If `CharSpan` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/CharSpan.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0468.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0469 (or finish at 1000)
