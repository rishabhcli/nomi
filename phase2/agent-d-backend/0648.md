# [D-0648] NumericReasoner: citation verifier false-positive elimination

| Field | Value |
|-------|-------|
| **Queue** | 648 / 1000 |
| **Agent** | D — Backend / Query Brain |
| **Phase** | Cross-Module Integration — Engine↔orchestrator↔UI contracts |
| **Priority** | P2 |
| **Seed** | `736514b95fd6` |
...
As a **staff backend engineer**, harden `NumericReasoner` using **citation verifier false-positive elimination** (seed `736514b95fd6`).

Deliverables:
- Identify ≥3 concrete failure modes in current `NumericReasoner` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `NumericReasoner`.
- If `NumericReasoner` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/NumericReasoner.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0648.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0649 (or finish at 1000)
