# [D-0948] QueryDecomposer: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `QueryDecomposer` using **citation verifier false-positive elimination** (seed `bd803ff9627d`).

Deliverables:
- Identify ≥3 concrete failure modes in current `QueryDecomposer` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `QueryDecomposer`.
- If `QueryDecomposer` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/QueryDecomposer.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0948.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0949 (or finish at 1000)
