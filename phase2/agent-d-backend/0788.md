# [D-0788] LLMSynthesizer: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `LLMSynthesizer` using **citation verifier false-positive elimination** (seed `9c27cc1ac3fd`).

Deliverables:
- Identify ≥3 concrete failure modes in current `LLMSynthesizer` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `LLMSynthesizer`.
- If `LLMSynthesizer` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/LLMSynthesizer.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0788.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0789 (or finish at 1000)
