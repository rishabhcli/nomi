...
As a **staff backend engineer**, harden `EntityExtractor` using **citation verifier false-positive elimination** (seed `9d74647f8bf1`).

Deliverables:
- Identify ≥3 concrete failure modes in current `EntityExtractor` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `EntityExtractor`.
- If `EntityExtractor` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/EntityExtractor.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0248.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0249 (or finish at 1000)
