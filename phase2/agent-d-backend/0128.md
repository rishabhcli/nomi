...
As a **staff backend engineer**, harden `EgressGuard` using **citation verifier false-positive elimination** (seed `201c528f8604`).

Deliverables:
- Identify ≥3 concrete failure modes in current `EgressGuard` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `EgressGuard`.
- If `EgressGuard` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/EgressGuard.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0128.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0129 (or finish at 1000)
