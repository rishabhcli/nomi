# [D-0888] ConflictDetector: citation verifier false-positive elimination

| Field | Value |
|-------|-------|
| **Queue** | 888 / 1000 |
| **Agent** | D — Backend / Query Brain |
| **Phase** | Beats-Siri Product Gates — Offline synthesis superiority proofs |
| **Priority** | P3 |
| **Seed** | `4bd0dfa47d4c` |
...
As a **staff backend engineer**, harden `ConflictDetector` using **citation verifier false-positive elimination** (seed `4bd0dfa47d4c`).

Deliverables:
- Identify ≥3 concrete failure modes in current `ConflictDetector` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `ConflictDetector`.
- If `ConflictDetector` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/ConflictDetector.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0888.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0889 (or finish at 1000)
