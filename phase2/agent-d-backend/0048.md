...
As a **staff backend engineer**, harden `Preferences` using **citation verifier false-positive elimination** (seed `a2a6fa7325b6`).

Deliverables:
- Identify ≥3 concrete failure modes in current `Preferences` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `Preferences`.
- If `Preferences` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/Preferences.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0048.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0049 (or finish at 1000)
