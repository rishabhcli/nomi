# [D-0808] CommandParser: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `CommandParser` using **citation verifier false-positive elimination** (seed `0a3572ae628e`).

Deliverables:
- Identify ≥3 concrete failure modes in current `CommandParser` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `CommandParser`.
- If `CommandParser` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/CommandParser.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0808.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0809 (or finish at 1000)
