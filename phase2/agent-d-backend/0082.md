...
As a **staff backend engineer**, harden `ScopeClassifier` using **concurrency stress under WorkScheduler** (seed `328e578e68af`).

Deliverables:
- Identify ≥3 concrete failure modes in current `ScopeClassifier` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `ScopeClassifier`.
- If `ScopeClassifier` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/ScopeClassifier.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0082.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0083 (or finish at 1000)
