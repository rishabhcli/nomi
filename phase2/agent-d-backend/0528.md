# [D-0528] SyncEngine: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `SyncEngine` using **citation verifier false-positive elimination** (seed `723f5269cd3d`).

Deliverables:
- Identify ≥3 concrete failure modes in current `SyncEngine` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `SyncEngine`.
- If `SyncEngine` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/SyncEngine.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0528.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0529 (or finish at 1000)
