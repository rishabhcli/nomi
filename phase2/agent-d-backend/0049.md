...
As a **staff backend engineer**, harden `Coverage` using **memory supersession race conditions** (seed `bf20cb7478d3`).

Deliverables:
- Identify ≥3 concrete failure modes in current `Coverage` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `Coverage`.
- If `Coverage` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/Coverage.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0049.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0050 (or finish at 1000)
