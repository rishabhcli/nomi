# [D-0748] QueryHistory: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `QueryHistory` using **citation verifier false-positive elimination** (seed `b1c486310cc1`).

Deliverables:
- Identify ≥3 concrete failure modes in current `QueryHistory` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `QueryHistory`.
- If `QueryHistory` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/QueryHistory.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0748.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0749 (or finish at 1000)
