# [D-0908] Confidence: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `Confidence` using **citation verifier false-positive elimination** (seed `fe773fcf0488`).

Deliverables:
- Identify ≥3 concrete failure modes in current `Confidence` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `Confidence`.
- If `Confidence` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/Confidence.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0908.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0909 (or finish at 1000)
