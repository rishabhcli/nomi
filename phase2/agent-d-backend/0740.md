# [D-0740] EgressGuard: mnemoctl JSON schema stability
...
As a **staff backend engineer**, harden `EgressGuard` using **mnemoctl JSON schema stability** (seed `0a20dcac8015`).
...
- [ ] Evidence: `phase2/evidence/D-0740.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0741 (or finish at 1000)
