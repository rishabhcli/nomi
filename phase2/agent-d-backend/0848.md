# [D-0848] AdaptiveEffort: citation verifier false-positive elimination
...
As a **staff backend engineer**, harden `AdaptiveEffort` using **citation verifier false-positive elimination** (seed `bd81bb8607d8`).

Deliverables:
- Identify ≥3 concrete failure modes in current `AdaptiveEffort` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `AdaptiveEffort`.
- If `AdaptiveEffort` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/AdaptiveEffort.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0848.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0849 (or finish at 1000)
