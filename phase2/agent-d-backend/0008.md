...
As a **staff backend engineer**, harden `SpanResolver` using **citation verifier false-positive elimination** (seed `3dcedb5dff2a`).

Deliverables:
- Identify ≥3 concrete failure modes in current `SpanResolver` implementation with file:line citations.
...
- Prove offline: loopback-only URLs, zero egress during `ask()` path touching `SpanResolver`.
- If `SpanResolver` emits `QueryEvent`s, assert `NotchReducer` can render every new event path.
...
**Targets:** `Sources/MnemoOrchestrator/SpanResolver.swift`, related tests, `NotchReducer.swift` if events change
...
- [ ] Evidence: `phase2/evidence/D-0008.md` with **raw** command output
- [ ] No `XCTAssertTrue(true)`; no batch automation scripts created
- [ ] Offline + loopback invariants preserved
- [ ] **Do not commit** — advance to D-0009 (or finish at 1000)
