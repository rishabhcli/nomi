# [G-1000] QA: invariant property tests on CommandParser
...
As a **staff QA/security engineer**, apply **invariant property tests** against `CommandParser` (seed `52712e32c2d0`).
...
**Targets:** `CommandParser.swift` + exhaustive test coverage expansion
...
- At **1000/1000 only**: one atomic commit per agent:
  - `git add -A && git commit -m "G: Phase 2 complete (G-0001..G-1000)"`
  - Then one push.
...
- [ ] Evidence: `phase2/evidence/G-1000.md` with **raw** command output
