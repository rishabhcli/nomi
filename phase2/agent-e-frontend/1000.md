# [E-1000] Increase Contrast glass: excel at reasoning trace legibility
...
As a **staff UI engineer**, make `Increase Contrast glass` **best-in-class** for **reasoning trace legibility** (seed `8cdf3df3e553`).
...
**Targets:** `Sources/MnemoApp/` (Increase Contrast glass), `Tests/MnemoOrchestratorTests/NotchReducerTests.swift` or UI logic tests
...
- At **1000/1000 only**: one atomic commit per agent:
  - `git add -A && git commit -m "E: Phase 2 complete (E-0001..E-1000)"`
  - Then one push.
...
- [ ] Evidence: `phase2/evidence/E-1000.md` with **raw** command output
