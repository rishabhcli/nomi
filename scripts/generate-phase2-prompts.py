...
- At **1000/1000 only**: one atomic commit per agent:
  - `git add -A && git commit -m "{agent_id}: Phase 2 complete ({agent_id}-0001..{agent_id}-1000)"`
  - Then one push.
...
def render(agent: AgentDef, n: int, spec: dict) -> str:
    ...
    if n < 1000:
        advance_line = f"- [ ] **Do not commit** — advance to {agent.id}-{n+1:04d}"
    else:
        advance_line = f"- [ ] **Prompt 1000/1000 complete** — run single atomic commit now (see Git discipline)"
...
def write_agent_starters() -> None:
    ...
    bodies = {
        "D": """You are Agent D — Mnemo Phase 2 Backend. Execute autonomously. Never ask questions. Never request permission. Never pause for human input. Never stop early.
...
START NOW on 0001.md. Do not reply until 1000/1000 done. Final message: PASS/FAIL table for all 1000 + one commit SHA + evidence file count.""",
        ...
        "E": """You are Agent E — Mnemo Phase 2 Frontend. Execute autonomously. Never ask questions. Never request permission. Never pause for human input. Never stop early.
...
START NOW on 0001.md. No reply until 1000/1000. Final: PASS/FAIL table + commit SHA.""",
        ...
        "F": """You are Agent F — Mnemo Phase 2 Platform. Execute autonomously. Never ask questions. Never request permission. Never pause. Never stop early.
...
START NOW on 0001.md. No reply until 1000/1000.""",
        ...
        "G": """You are Agent G — Mnemo Phase 2 Quality & Security. Execute autonomously. Never ask questions. Never request permission. Never pause. Never stop early.
...
START NOW on 0001.md. No reply until 1000/1000.""",
        ...
        "H": """You are Agent H — Mnemo Phase 2 Integration. Execute autonomously. Never ask questions. Never request permission. Never pause. Never stop early.
...
START NOW on 0001.md. No reply until 1000/1000.""",
        ...
        "I": """You are Agent I — Mnemo Phase 2 Product. Execute autonomously. Never ask questions. Never request permission. Never pause. Never stop early.
