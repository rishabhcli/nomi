...
/// D-0577: AsyncStream cancellation for Ingestion (seed b4c381a6926e).
final class D0577IngestionTests: XCTestCase {
    private let seed = "b4c381a6926e"

    func testAsync_cancelProof() async {
        XCTAssertTrue(await Ingestion.asyncStreamCancelProof())
    }

    func testAsync_cancelSafe() {
        XCTAssertTrue(Ingestion.asyncStreamCancelSafe())
    }
