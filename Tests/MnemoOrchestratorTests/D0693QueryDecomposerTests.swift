...
/// D-0693: profile preamble staleness for QueryDecomposer (seed 7090fc83c221).
final class D0693QueryDecomposerTests: XCTestCase {
    private let seed = "7090fc83c221"

    func testProfile_staleFilter() {
        let profile = Profile(statics: ["old"], dynamics: [], memories: [])
        XCTAssertTrue(QueryDecomposer.filtersStaleProfilePreamble(profile, active: false))
        XCTAssertTrue(QueryDecomposer.filtersStaleProfilePreamble(profile, active: true))
    }
