# [A-341] Latency: QueryRewriter.swift scheduling
...
Concurrency/latency in `QueryRewriter.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryRewriter.swift` + `WorkScheduler.swift`
