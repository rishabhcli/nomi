# [A-329] Latency: IngestGate.swift scheduling
...
Concurrency/latency in `IngestGate.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/IngestGate.swift` + `WorkScheduler.swift`
