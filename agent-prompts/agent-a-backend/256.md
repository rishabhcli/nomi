# [A-256] Consolidation: Preferences.swift dreaming safety
...
Consolidation pass in `Preferences.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `SpanResolverTests.swift`.
