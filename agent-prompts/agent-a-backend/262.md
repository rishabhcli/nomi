# [A-262] Consolidation: Router.swift dreaming safety
...
Consolidation pass in `Router.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `QueryServiceTests.swift`.
