# [A-120] Query lifecycle: Ingestion.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 120 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `Ingestion.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `RouterTests.swift` covering prompt #120's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/Ingestion.swift` + `NotchReducer.swift`
