# [A-220] Memory: LLMHopPlanner.swift dynamics
...
Strengthen memory dynamics in `LLMHopPlanner.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `SchedulerTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `SchedulerTests.swift`
