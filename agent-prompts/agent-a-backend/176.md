# [A-176] Ingestion: MemoryDynamics.swift reliability
...
Improve ingestion/sync in `MemoryDynamics.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `QueryLifecycleTests.swift`.
