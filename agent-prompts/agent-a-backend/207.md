# [A-207] Memory: ActionExtractor.swift dynamics
...
Strengthen memory dynamics in `ActionExtractor.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `RouterTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `RouterTests.swift`
