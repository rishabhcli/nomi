# [A-160] Grounding: EvidenceGathering.swift citation integrity
...
Raise retrieval/citation quality in `EvidenceGathering.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `EgressGuardTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/EvidenceGathering.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `EgressGuardTests.swift`
