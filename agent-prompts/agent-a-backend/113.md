# [A-113] Query lifecycle: CharSpan.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 113 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `CharSpan.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `DocumentsTests.swift` covering prompt #113's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/CharSpan.swift` + `NotchReducer.swift`
