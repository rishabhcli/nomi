# [A-260] Consolidation: CorpusSuggester.swift dreaming safety
...
Consolidation pass in `CorpusSuggester.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `CommandParserTests.swift`.
