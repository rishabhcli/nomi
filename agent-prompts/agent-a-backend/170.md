# [A-170] Ingestion: Prompt.swift reliability
...
Improve ingestion/sync in `Prompt.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `CharSpanTests.swift`.
