# [A-090] Query lifecycle: TimelineBuilder.swift event completeness
...
Harden query lifecycle in `TimelineBuilder.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `StateMachineTests.swift` covering prompt #90's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/TimelineBuilder.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/StateMachineTests.swift` or `QueryLifecycleTests.swift`
