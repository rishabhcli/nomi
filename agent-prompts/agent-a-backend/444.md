# [A-444] Beats-Siri: NotchReducer.swift offline synthesis
...
Beats-Siri gate for `NotchReducer.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #444.
