# [A-332] Latency: MemoryDynamics.swift scheduling
...
Concurrency/latency in `MemoryDynamics.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/MemoryDynamics.swift` + `WorkScheduler.swift`
