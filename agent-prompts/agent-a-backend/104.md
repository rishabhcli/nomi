# [A-104] Query lifecycle: CorpusSuggester.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 104 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `CorpusSuggester.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `SchedulerTests.swift` covering prompt #104's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/CorpusSuggester.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/SchedulerTests.swift` or `QueryLifecycleTests.swift`
