# [A-236] Memory: NotchReducer.swift dynamics
...
Strengthen memory dynamics in `NotchReducer.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `RouterTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
