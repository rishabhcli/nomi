# [A-119] Query lifecycle: OllamaClient.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 119 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `OllamaClient.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `StateMachineTests.swift` covering prompt #119's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/OllamaClient.swift` + `NotchReducer.swift`
