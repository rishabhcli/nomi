# [A-339] Latency: WorkScheduler.swift scheduling
...
Concurrency/latency in `WorkScheduler.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/WorkScheduler.swift` + `WorkScheduler.swift`
