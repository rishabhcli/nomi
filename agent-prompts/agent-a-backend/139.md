# [A-139] Grounding: PersonalRanker.swift citation integrity
...
Raise retrieval/citation quality in `PersonalRanker.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `HopPlannerTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/PersonalRanker.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `HopPlannerTests.swift`
