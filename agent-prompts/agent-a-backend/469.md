# [A-469] Beats-Siri: QueryService.swift offline synthesis
...
Beats-Siri gate for `QueryService.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #469.
