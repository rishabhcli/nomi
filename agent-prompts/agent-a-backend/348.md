# [A-348] Latency: NumericReasoner.swift scheduling
...
Concurrency/latency in `NumericReasoner.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/NumericReasoner.swift` + `WorkScheduler.swift`
