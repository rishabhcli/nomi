# [A-135] Grounding: ScopeClassifier.swift citation integrity
...
Raise retrieval/citation quality in `ScopeClassifier.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `HelpfulnessTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/ScopeClassifier.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `HelpfulnessTests.swift`
