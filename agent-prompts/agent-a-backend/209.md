# [A-209] Memory: QueryService.swift dynamics
...
Strengthen memory dynamics in `QueryService.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `EngineClientTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
