# [A-151] Grounding: Digest.swift citation integrity
...
Raise retrieval/citation quality in `Digest.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `EngineClientTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/Digest.swift`
