# [A-149] Grounding: MediaCompanion.swift citation integrity
...
Raise retrieval/citation quality in `MediaCompanion.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `RouterTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/MediaCompanion.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `RouterTests.swift`
