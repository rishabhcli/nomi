# [A-117] Query lifecycle: ContextAssembler.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 117 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `ContextAssembler.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `QueryServiceTests.swift` covering prompt #117's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/ContextAssembler.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/QueryServiceTests.swift` or `QueryLifecycleTests.swift`
