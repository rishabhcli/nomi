# [A-255] Consolidation: Digest.swift dreaming safety
...
Consolidation pass in `Digest.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `HopPlannerTests.swift`.
