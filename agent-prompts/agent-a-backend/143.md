# [A-143] Grounding: ResponseStyle.swift citation integrity
...
Raise retrieval/citation quality in `ResponseStyle.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `ProfileTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/ResponseStyle.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `ProfileTests.swift`
