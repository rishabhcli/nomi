# [A-096] Query lifecycle: EntityExtractor.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 96 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `EntityExtractor.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `IngestIndexTests.swift` covering prompt #96's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/EntityExtractor.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `QueryLifecycleTests.swift`
