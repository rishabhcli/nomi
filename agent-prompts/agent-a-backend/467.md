# [A-467] Beats-Siri: ActionExtractor.swift offline synthesis
...
Beats-Siri gate for `ActionExtractor.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #467.
