# [A-186] Ingestion: QueryDecomposer.swift reliability
...
Improve ingestion/sync in `QueryDecomposer.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `MemoryDynamicsTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `MemoryDynamicsTests.swift`
