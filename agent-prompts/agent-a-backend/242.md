# [A-242] Consolidation: QueryHistory.swift dreaming safety
...
Consolidation pass in `QueryHistory.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `IngestGateTests.swift`.
