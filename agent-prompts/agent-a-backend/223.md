# [A-223] Memory: OllamaClient.swift dynamics
...
Strengthen memory dynamics in `OllamaClient.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `SmarterThanSiriTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
**Primary targets:** `Sources/MnemoOrchestrator/OllamaClient.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `SmarterThanSiriTests.swift`
