# [A-171] Ingestion: OllamaClient.swift reliability
...
Improve ingestion/sync in `OllamaClient.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `DocumentsTests.swift`.
