# [A-084] Query lifecycle: AdaptiveEffort.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 84 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `AdaptiveEffort.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `DocumentsTests.swift` covering prompt #84's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/AdaptiveEffort.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/DocumentsTests.swift` or `QueryLifecycleTests.swift`
