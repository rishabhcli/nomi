# [A-388] CLI: expose Inspector.swift via mnemoctl
...
Wire `Inspector.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `Inspector.swift`
