# [A-398] CLI: expose QueryHistory.swift via mnemoctl
...
Wire `QueryHistory.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `QueryHistory.swift`
