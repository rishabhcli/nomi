# [A-363] CLI: expose ActionExtractor.swift via mnemoctl
...
Wire `ActionExtractor.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `ActionExtractor.swift`
