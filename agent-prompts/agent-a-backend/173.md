# [A-173] Ingestion: IngestGate.swift reliability
...
Improve ingestion/sync in `IngestGate.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `CommandParserTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `CommandParserTests.swift`
