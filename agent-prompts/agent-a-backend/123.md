# [A-123] Grounding: ContentHash.swift citation integrity
...
Raise retrieval/citation quality in `ContentHash.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `EngineIntegrationTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/ContentHash.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `EngineIntegrationTests.swift`
