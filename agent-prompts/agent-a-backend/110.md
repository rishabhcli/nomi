# [A-110] Query lifecycle: EngineIntegration.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 110 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `EngineIntegration.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `HopPlannerTests.swift` covering prompt #110's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/EngineIntegration.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/HopPlannerTests.swift` or `QueryLifecycleTests.swift`
