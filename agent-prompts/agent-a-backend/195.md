# [A-195] Ingestion: ResponseStyle.swift reliability
...
Improve ingestion/sync in `ResponseStyle.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `IntelligenceTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `IntelligenceTests.swift`
