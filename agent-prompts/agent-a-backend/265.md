# [A-265] Consolidation: EngineClient.swift dreaming safety
...
Consolidation pass in `EngineClient.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `RouterTests.swift`.
