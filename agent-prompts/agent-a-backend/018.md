# [A-018] Foundation audit: SyncEngine.swift
...
Verify `SyncEngine.swift` never constructs a URL with a non-loopback host. Add invariant assertion test in `InvariantTests` or `BugfixRegressionTests.swift`.
