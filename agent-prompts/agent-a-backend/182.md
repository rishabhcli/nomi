# [A-182] Ingestion: EgressGuard.swift reliability
...
Improve ingestion/sync in `EgressGuard.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `AgenticGrepTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `AgenticGrepTests.swift`
