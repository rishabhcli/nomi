# [A-091] Query lifecycle: ResponseStyle.swift event completeness
...
Harden query lifecycle in `ResponseStyle.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `RouterTests.swift` covering prompt #91's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/ResponseStyle.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/RouterTests.swift` or `QueryLifecycleTests.swift`
