# [A-161] Ingestion: EngineClient.swift reliability
...
Improve ingestion/sync in `EngineClient.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `EgressHostAndCitationTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/EngineClient.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `EgressHostAndCitationTests.swift`
