# [A-180] Ingestion: Inspector.swift reliability
...
Improve ingestion/sync in `Inspector.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `EngineClientTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `EngineClientTests.swift`
