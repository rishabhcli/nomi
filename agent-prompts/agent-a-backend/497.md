# [A-497] Beats-Siri: QueryRewriter.swift offline synthesis
...
Beats-Siri gate for `QueryRewriter.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #497.
