# [A-387] CLI: expose LLMSynthesizer.swift via mnemoctl
...
Wire `LLMSynthesizer.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `LLMSynthesizer.swift`
