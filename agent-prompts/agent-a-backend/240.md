# [A-240] Memory: AdaptiveEffort.swift dynamics
...
Strengthen memory dynamics in `AdaptiveEffort.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `AgenticGrepTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
