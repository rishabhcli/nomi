# [A-191] Ingestion: PersonalRanker.swift reliability
...
Improve ingestion/sync in `PersonalRanker.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `SchedulerTests.swift`.
