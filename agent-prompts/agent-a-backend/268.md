# [A-268] Consolidation: SpanResolver.swift dreaming safety
...
Consolidation pass in `SpanResolver.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `EngineIntegrationTests.swift`.
