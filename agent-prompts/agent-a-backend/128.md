# [A-128] Grounding: Inspector.swift citation integrity
...
Raise retrieval/citation quality in `Inspector.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `MemoryDynamicsTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/Inspector.swift`
