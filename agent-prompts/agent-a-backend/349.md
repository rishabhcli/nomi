# [A-349] Latency: TimeWindow.swift scheduling
...
Concurrency/latency in `TimeWindow.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/TimeWindow.swift` + `WorkScheduler.swift`
