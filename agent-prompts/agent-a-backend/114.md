# [A-114] Query lifecycle: AgenticGrep.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 114 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `AgenticGrep.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `ProfileTests.swift` covering prompt #114's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/AgenticGrep.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/ProfileTests.swift` or `QueryLifecycleTests.swift`
