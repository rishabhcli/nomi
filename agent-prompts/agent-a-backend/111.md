# [A-111] Query lifecycle: CitationVerifier.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 111 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `CitationVerifier.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `SpanResolverTests.swift` covering prompt #111's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/CitationVerifier.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/SpanResolverTests.swift` or `QueryLifecycleTests.swift`
