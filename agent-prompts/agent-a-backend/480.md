# [A-480] Beats-Siri: LLMHopPlanner.swift offline synthesis
...
Beats-Siri gate for `LLMHopPlanner.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #480.
