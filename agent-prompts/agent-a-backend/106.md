# [A-106] Query lifecycle: Router.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 106 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `Router.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `HelpfulnessTests.swift` covering prompt #106's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/Router.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/HelpfulnessTests.swift` or `QueryLifecycleTests.swift`
