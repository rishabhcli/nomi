# [A-184] Ingestion: NotchReducer.swift reliability
...
Improve ingestion/sync in `NotchReducer.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `IngestGateTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `IngestGateTests.swift`
