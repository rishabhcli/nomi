# [A-456] Beats-Siri: FollowUp.swift offline synthesis
...
Beats-Siri gate for `FollowUp.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #456.
