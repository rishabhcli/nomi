# [A-274] Consolidation: Prompt.swift dreaming safety
...
Consolidation pass in `Prompt.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `ConsolidationTests.swift`.
