# [A-112] Query lifecycle: SpanResolver.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 112 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `SpanResolver.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `CharSpanTests.swift` covering prompt #112's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/SpanResolver.swift` + `NotchReducer.swift`
