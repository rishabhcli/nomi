# [A-230] Memory: Consolidation.swift dynamics
...
Strengthen memory dynamics in `Consolidation.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `ProfileTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
