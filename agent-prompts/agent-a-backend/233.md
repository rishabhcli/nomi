# [A-233] Memory: Profile.swift dynamics
...
Strengthen memory dynamics in `Profile.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `QueryServiceTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
