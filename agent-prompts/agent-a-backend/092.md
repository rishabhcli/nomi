# [A-092] Query lifecycle: FollowUp.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 92 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `FollowUp.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `CitationVerifierTests.swift` covering prompt #92's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/FollowUp.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `QueryLifecycleTests.swift`
