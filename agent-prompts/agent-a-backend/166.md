# [A-166] Ingestion: AgenticGrep.swift reliability
...
Improve ingestion/sync in `AgenticGrep.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `IntelligenceTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/AgenticGrep.swift`
