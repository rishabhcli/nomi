# [A-354] Latency: Provenance.swift scheduling
...
Concurrency/latency in `Provenance.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/Provenance.swift` + `WorkScheduler.swift`
