# [A-107] Query lifecycle: RouterEscalator.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 107 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `RouterEscalator.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `SmarterThanSiriTests.swift` covering prompt #107's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/RouterEscalator.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/SmarterThanSiriTests.swift` or `QueryLifecycleTests.swift`
