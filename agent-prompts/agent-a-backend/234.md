# [A-234] Memory: EgressGuard.swift dynamics
...
Strengthen memory dynamics in `EgressGuard.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `QueryLifecycleTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
