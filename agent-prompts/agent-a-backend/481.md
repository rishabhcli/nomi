# [A-481] Beats-Siri: ContextAssembler.swift offline synthesis
...
Beats-Siri gate for `ContextAssembler.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #481.
