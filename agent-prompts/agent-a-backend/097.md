# [A-097] Query lifecycle: MediaCompanion.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 97 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `MediaCompanion.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `IngestGateTests.swift` covering prompt #97's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/MediaCompanion.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestGateTests.swift` or `QueryLifecycleTests.swift`
