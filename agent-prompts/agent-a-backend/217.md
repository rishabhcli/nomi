# [A-217] Memory: CharSpan.swift dynamics
...
Strengthen memory dynamics in `CharSpan.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `InspectorTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `InspectorTests.swift`
