# [A-248] Consolidation: FollowUp.swift dreaming safety
...
Consolidation pass in `FollowUp.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `EgressHostAndCitationTests.swift`.
