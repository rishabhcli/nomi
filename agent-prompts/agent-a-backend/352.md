# [A-352] Latency: FollowUp.swift scheduling
...
Concurrency/latency in `FollowUp.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/FollowUp.swift` + `WorkScheduler.swift`
