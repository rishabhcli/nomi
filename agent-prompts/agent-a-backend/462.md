# [A-462] Beats-Siri: LocalExtractor.swift offline synthesis
...
Beats-Siri gate for `LocalExtractor.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #462.
