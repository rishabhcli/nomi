# [A-229] Memory: ConflictDetector.swift dynamics
...
Strengthen memory dynamics in `ConflictDetector.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `DocumentsTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `DocumentsTests.swift`
