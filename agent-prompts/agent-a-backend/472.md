# [A-472] Beats-Siri: EvidenceGathering.swift offline synthesis

| Field | Value |
|-------|-------|
| **Queue position** | 472 of 500 (chronological — do not skip) |
| **Agent** | A — Backend / Orchestrator |
| **Phase** | Polish & Beats-Siri — Helpfulness, smarter-than-Siri criteria, final gates |
| **Priority** | P3 |
...
Beats-Siri gate for `EvidenceGathering.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #472.
