# [A-181] Ingestion: Profile.swift reliability
...
Improve ingestion/sync in `Profile.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `EngineIntegrationTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `EngineIntegrationTests.swift`
