# [A-116] Query lifecycle: LLMHopPlanner.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 116 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `LLMHopPlanner.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `NotchReducerTests.swift` covering prompt #116's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/LLMHopPlanner.swift` + `NotchReducer.swift`
