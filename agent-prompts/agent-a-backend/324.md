# [A-324] Latency: LLMHopPlanner.swift scheduling
...
Concurrency/latency in `LLMHopPlanner.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/LLMHopPlanner.swift` + `WorkScheduler.swift`
