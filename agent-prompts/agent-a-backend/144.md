# [A-144] Grounding: FollowUp.swift citation integrity
...
Raise retrieval/citation quality in `FollowUp.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `CommandParserTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/FollowUp.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `CommandParserTests.swift`
