# [A-103] Query lifecycle: ActionExtractor.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 103 of 500 (chronological — do not skip) |
| **Agent** | A — Backend / Orchestrator |
| **Phase** | Query Lifecycle — Router, decomposition, effort, state machine events |
| **Priority** | P1 |
...
Harden query lifecycle in `ActionExtractor.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `EgressHostAndCitationTests.swift` covering prompt #103's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/ActionExtractor.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/EgressHostAndCitationTests.swift` or `QueryLifecycleTests.swift`
