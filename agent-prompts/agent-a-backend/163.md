# [A-163] Ingestion: CitationVerifier.swift reliability

| Field | Value |
|-------|-------|
| **Queue position** | 163 of 500 (chronological — do not skip) |
| **Agent** | A — Backend / Orchestrator |
| **Phase** | Ingestion & Sync — IngestIndex, gates, self-heal, content hash |
| **Priority** | P1 |
...
Improve ingestion/sync in `CitationVerifier.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `BugfixRegressionTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/CitationVerifier.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `BugfixRegressionTests.swift`
