# [A-201] Memory: MediaCompanion.swift dynamics
...
Strengthen memory dynamics in `MediaCompanion.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `ProfileTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `ProfileTests.swift`
