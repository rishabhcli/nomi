# [A-202] Memory: LocalExtractor.swift dynamics
...
Strengthen memory dynamics in `LocalExtractor.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `CommandParserTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
