# [A-137] Grounding: AnswerCache.swift citation integrity
...
Raise retrieval/citation quality in `AnswerCache.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `IntelligenceTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/AnswerCache.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `IntelligenceTests.swift`
