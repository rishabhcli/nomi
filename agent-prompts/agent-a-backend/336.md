# [A-336] Latency: Inspector.swift scheduling
...
Concurrency/latency in `Inspector.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/Inspector.swift` + `WorkScheduler.swift`
