# [A-350] Latency: TimelineBuilder.swift scheduling
...
Concurrency/latency in `TimelineBuilder.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/TimelineBuilder.swift` + `WorkScheduler.swift`
