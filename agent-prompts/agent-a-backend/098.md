# [A-098] Query lifecycle: LocalExtractor.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 98 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `LocalExtractor.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `SyncEngineTests.swift` covering prompt #98's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/LocalExtractor.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/SyncEngineTests.swift` or `QueryLifecycleTests.swift`
