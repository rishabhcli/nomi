# [A-187] Ingestion: ScopeClassifier.swift reliability
...
Improve ingestion/sync in `ScopeClassifier.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `ConsolidationTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `ConsolidationTests.swift`
