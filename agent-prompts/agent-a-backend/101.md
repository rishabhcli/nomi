# [A-101] Query lifecycle: Coverage.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 101 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `Coverage.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `InspectorTests.swift` covering prompt #101's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/Coverage.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/InspectorTests.swift` or `QueryLifecycleTests.swift`
