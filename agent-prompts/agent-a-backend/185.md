# [A-185] Ingestion: QueryRewriter.swift reliability
...
Improve ingestion/sync in `QueryRewriter.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `SyncEngineTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `SyncEngineTests.swift`
