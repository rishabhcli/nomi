# [A-118] Query lifecycle: Prompt.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 118 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `Prompt.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `QueryLifecycleTests.swift` covering prompt #118's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/Prompt.swift` + `NotchReducer.swift`
