# [A-148] Grounding: EntityExtractor.swift citation integrity
...
Raise retrieval/citation quality in `EntityExtractor.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `StateMachineTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/EntityExtractor.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `StateMachineTests.swift`
