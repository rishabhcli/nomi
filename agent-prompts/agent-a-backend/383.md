# [A-383] CLI: expose ContentHash.swift via mnemoctl
...
Wire `ContentHash.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `ContentHash.swift`
