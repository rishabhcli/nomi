# [A-351] Latency: ResponseStyle.swift scheduling
...
Concurrency/latency in `ResponseStyle.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/ResponseStyle.swift` + `WorkScheduler.swift`
