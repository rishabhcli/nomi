# [A-251] Consolidation: CommandParser.swift dreaming safety
...
Consolidation pass in `CommandParser.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `HelpfulnessTests.swift`.
