# [A-214] Memory: EngineIntegration.swift dynamics
...
Strengthen memory dynamics in `EngineIntegration.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `SyncEngineTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
**Primary targets:** `Sources/MnemoOrchestrator/EngineIntegration.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `SyncEngineTests.swift`
