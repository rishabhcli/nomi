# [A-328] Latency: Ingestion.swift scheduling
...
Concurrency/latency in `Ingestion.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/Ingestion.swift` + `WorkScheduler.swift`
