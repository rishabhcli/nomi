# [A-193] Ingestion: TimeWindow.swift reliability
...
Improve ingestion/sync in `TimeWindow.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `HelpfulnessTests.swift`.
