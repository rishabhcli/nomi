# [A-127] Grounding: LLMSynthesizer.swift citation integrity
...
Raise retrieval/citation quality in `LLMSynthesizer.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `SyncEngineTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/LLMSynthesizer.swift`
