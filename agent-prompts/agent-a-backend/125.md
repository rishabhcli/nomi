# [A-125] Grounding: ConflictDetector.swift citation integrity
...
Raise retrieval/citation quality in `ConflictDetector.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `IngestIndexTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/ConflictDetector.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `IngestIndexTests.swift`
