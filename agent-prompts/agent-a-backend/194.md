# [A-194] Ingestion: TimelineBuilder.swift reliability
...
Improve ingestion/sync in `TimelineBuilder.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `SmarterThanSiriTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `SmarterThanSiriTests.swift`
