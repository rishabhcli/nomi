# [A-254] Consolidation: LocalExtractor.swift dreaming safety
...
Consolidation pass in `LocalExtractor.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `ExpressivenessTests.swift`.
