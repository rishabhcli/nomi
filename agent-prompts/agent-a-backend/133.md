# [A-133] Grounding: QueryRewriter.swift citation integrity
...
Raise retrieval/citation quality in `QueryRewriter.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `SchedulerTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryRewriter.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `SchedulerTests.swift`
