# [A-189] Ingestion: AnswerCache.swift reliability
...
Improve ingestion/sync in `AnswerCache.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `EgressGuardTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `EgressGuardTests.swift`
