# [A-134] Grounding: QueryDecomposer.swift citation integrity
...
Raise retrieval/citation quality in `QueryDecomposer.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `BugfixRegressionTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryDecomposer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `BugfixRegressionTests.swift`
