# [A-216] Memory: SpanResolver.swift dynamics
...
Strengthen memory dynamics in `SpanResolver.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `ConsolidationTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
**Primary targets:** `Sources/MnemoOrchestrator/SpanResolver.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `ConsolidationTests.swift`
