# [A-150] Grounding: LocalExtractor.swift citation integrity
...
Raise retrieval/citation quality in `LocalExtractor.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `CitationVerifierTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/LocalExtractor.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `CitationVerifierTests.swift`
