# [A-188] Ingestion: AdaptiveEffort.swift reliability
...
Improve ingestion/sync in `AdaptiveEffort.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `InspectorTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `InspectorTests.swift`
