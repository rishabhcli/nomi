# [A-245] Consolidation: TimeWindow.swift dreaming safety
...
Consolidation pass in `TimeWindow.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `ConsolidationTests.swift`.
