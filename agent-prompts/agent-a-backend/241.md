# [A-241] Consolidation: AnswerCache.swift dreaming safety
...
Consolidation pass in `AnswerCache.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `IngestIndexTests.swift`.
