# [A-200] Ingestion: EntityExtractor.swift reliability
...
Improve ingestion/sync in `EntityExtractor.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `DocumentsTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `DocumentsTests.swift`
