# [A-231] Memory: LLMSynthesizer.swift dynamics
...
Strengthen memory dynamics in `LLMSynthesizer.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `CommandParserTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `CommandParserTests.swift`
