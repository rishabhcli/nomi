# [A-221] Memory: ContextAssembler.swift dynamics
...
Strengthen memory dynamics in `ContextAssembler.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `BugfixRegressionTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
**Primary targets:** `Sources/MnemoOrchestrator/ContextAssembler.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `BugfixRegressionTests.swift`
