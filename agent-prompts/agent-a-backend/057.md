# [A-057] Beats-Siri: EngineClient.swift offline synthesis
...
Beats-Siri gate for `EngineClient.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #57.
