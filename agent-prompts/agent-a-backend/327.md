# [A-327] Latency: OllamaClient.swift scheduling
...
Concurrency/latency in `OllamaClient.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/OllamaClient.swift` + `WorkScheduler.swift`
