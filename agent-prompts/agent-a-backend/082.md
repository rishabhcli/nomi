# [A-082] Query lifecycle: QueryDecomposer.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 82 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `QueryDecomposer.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `SpanResolverTests.swift` covering prompt #82's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryDecomposer.swift` + `NotchReducer.swift`
