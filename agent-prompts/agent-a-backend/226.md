# [A-226] Memory: SyncEngine.swift dynamics
...
Strengthen memory dynamics in `SyncEngine.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `HopPlannerTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `HopPlannerTests.swift`
