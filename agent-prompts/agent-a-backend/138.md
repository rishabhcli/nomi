# [A-138] Grounding: QueryHistory.swift citation integrity
...
Raise retrieval/citation quality in `QueryHistory.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `ExpressivenessTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryHistory.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `ExpressivenessTests.swift`
