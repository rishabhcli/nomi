# [A-392] CLI: expose NotchReducer.swift via mnemoctl
...
Wire `NotchReducer.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `NotchReducer.swift`
