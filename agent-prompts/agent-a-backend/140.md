# [A-140] Grounding: NumericReasoner.swift citation integrity
...
Raise retrieval/citation quality in `NumericReasoner.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `SpanResolverTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/NumericReasoner.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `SpanResolverTests.swift`
