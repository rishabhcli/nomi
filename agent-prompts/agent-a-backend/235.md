# [A-235] Memory: WorkScheduler.swift dynamics
...
Strengthen memory dynamics in `WorkScheduler.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `StateMachineTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `StateMachineTests.swift`
