# [A-169] Ingestion: ContextAssembler.swift reliability
...
Improve ingestion/sync in `ContextAssembler.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `SpanResolverTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `SpanResolverTests.swift`
