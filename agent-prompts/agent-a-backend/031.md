# [A-031] Foundation audit: ScopeClassifier.swift
...
Read `ScopeClassifier.swift` end-to-end and document every public entry point in a code comment block at file top (max 15 lines). Add one missing unit test in `QueryLifecycleTests.swift` for an uncovered branch.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/QueryLifecycleTests.swift`
