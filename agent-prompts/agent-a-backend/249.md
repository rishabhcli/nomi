# [A-249] Consolidation: Confidence.swift dreaming safety
...
Consolidation pass in `Confidence.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `SchedulerTests.swift`.
