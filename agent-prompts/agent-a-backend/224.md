# [A-224] Memory: Ingestion.swift dynamics
...
Strengthen memory dynamics in `Ingestion.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `IntelligenceTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
**Primary targets:** `Sources/MnemoOrchestrator/Ingestion.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `IntelligenceTests.swift`
