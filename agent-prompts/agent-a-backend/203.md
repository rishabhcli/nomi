# [A-203] Memory: Digest.swift dynamics
...
Strengthen memory dynamics in `Digest.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `NotchReducerTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
