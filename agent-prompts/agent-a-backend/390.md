# [A-390] CLI: expose EgressGuard.swift via mnemoctl
...
Wire `EgressGuard.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `EgressGuard.swift`
