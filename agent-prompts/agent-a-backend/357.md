# [A-357] Latency: MediaCompanion.swift scheduling
...
Concurrency/latency in `MediaCompanion.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/MediaCompanion.swift` + `WorkScheduler.swift`
