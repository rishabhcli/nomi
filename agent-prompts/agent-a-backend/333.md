# [A-333] Latency: ConflictDetector.swift scheduling
...
Concurrency/latency in `ConflictDetector.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/ConflictDetector.swift` + `WorkScheduler.swift`
