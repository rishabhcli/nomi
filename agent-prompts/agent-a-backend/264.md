# [A-264] Consolidation: EvidenceGathering.swift dreaming safety
...
Consolidation pass in `EvidenceGathering.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `StateMachineTests.swift`.
