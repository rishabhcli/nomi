# [A-093] Query lifecycle: Confidence.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 93 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `Confidence.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `EngineClientTests.swift` covering prompt #93's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/Confidence.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/EngineClientTests.swift` or `QueryLifecycleTests.swift`
