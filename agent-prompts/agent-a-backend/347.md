# [A-347] Latency: PersonalRanker.swift scheduling
...
Concurrency/latency in `PersonalRanker.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/PersonalRanker.swift` + `WorkScheduler.swift`
