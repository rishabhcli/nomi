# [A-322] Latency: AgenticGrep.swift scheduling
...
Concurrency/latency in `AgenticGrep.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/AgenticGrep.swift` + `WorkScheduler.swift`
