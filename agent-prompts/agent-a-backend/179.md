# [A-179] Ingestion: LLMSynthesizer.swift reliability
...
Improve ingestion/sync in `LLMSynthesizer.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `CitationVerifierTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `CitationVerifierTests.swift`
