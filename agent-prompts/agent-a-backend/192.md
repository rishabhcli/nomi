# [A-192] Ingestion: NumericReasoner.swift reliability
...
Improve ingestion/sync in `NumericReasoner.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `BugfixRegressionTests.swift`.
