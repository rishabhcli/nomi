# [A-081] Query lifecycle: QueryRewriter.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 81 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `QueryRewriter.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `HopPlannerTests.swift` covering prompt #81's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryRewriter.swift` + `NotchReducer.swift`
