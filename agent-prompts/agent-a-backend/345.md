# [A-345] Latency: AnswerCache.swift scheduling
...
Concurrency/latency in `AnswerCache.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/AnswerCache.swift` + `WorkScheduler.swift`
