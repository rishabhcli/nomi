# [A-346] Latency: QueryHistory.swift scheduling
...
Concurrency/latency in `QueryHistory.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryHistory.swift` + `WorkScheduler.swift`
