# [A-108] Query lifecycle: EvidenceGathering.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 108 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `EvidenceGathering.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `IntelligenceTests.swift` covering prompt #108's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/EvidenceGathering.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IntelligenceTests.swift` or `QueryLifecycleTests.swift`
