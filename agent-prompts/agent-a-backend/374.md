# [A-374] CLI: expose AgenticGrep.swift via mnemoctl
...
Wire `AgenticGrep.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `AgenticGrep.swift`
