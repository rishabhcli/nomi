# [A-165] Ingestion: CharSpan.swift reliability
...
Improve ingestion/sync in `CharSpan.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `SmarterThanSiriTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/CharSpan.swift`
