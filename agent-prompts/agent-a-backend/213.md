# [A-213] Memory: EngineClient.swift dynamics
...
Strengthen memory dynamics in `EngineClient.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `IngestGateTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `IngestGateTests.swift`
