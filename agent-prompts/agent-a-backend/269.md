# [A-269] Consolidation: CharSpan.swift dreaming safety
...
Consolidation pass in `CharSpan.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `AgenticGrepTests.swift`.
