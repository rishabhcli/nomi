# [A-483] Beats-Siri: OllamaClient.swift offline synthesis
...
Beats-Siri gate for `OllamaClient.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #483.
