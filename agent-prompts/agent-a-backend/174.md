# [A-174] Ingestion: SyncEngine.swift reliability
...
Improve ingestion/sync in `SyncEngine.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `NotchReducerTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `NotchReducerTests.swift`
