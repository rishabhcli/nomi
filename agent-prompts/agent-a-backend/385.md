# [A-385] CLI: expose ConflictDetector.swift via mnemoctl
...
Wire `ConflictDetector.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `ConflictDetector.swift`
