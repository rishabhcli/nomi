# [A-060] Beats-Siri: SpanResolver.swift offline synthesis
...
Beats-Siri gate for `SpanResolver.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #60.
