# [A-370] CLI: expose EngineIntegration.swift via mnemoctl

| Field | Value |
|-------|-------|
| **Queue position** | 370 of 500 (chronological — do not skip) |
| **Agent** | A — Backend / Orchestrator |
| **Phase** | CLI & Integration — mnemoctl subcommands, engine wiring, fixtures |
| **Priority** | P3 |
...
Wire `EngineIntegration.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `EngineIntegration.swift`
...
- [ ] New/changed behavior has automated test coverage: Run `.build/debug/mnemoctl ask` with fixture corpus offline
