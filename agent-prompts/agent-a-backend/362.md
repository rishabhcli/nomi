# [A-362] CLI: expose Highlight.swift via mnemoctl
...
Wire `Highlight.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `Highlight.swift`
