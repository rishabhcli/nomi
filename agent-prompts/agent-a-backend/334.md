# [A-334] Latency: Consolidation.swift scheduling
...
Concurrency/latency in `Consolidation.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/Consolidation.swift` + `WorkScheduler.swift`
