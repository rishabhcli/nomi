# [A-088] Query lifecycle: NumericReasoner.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 88 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `NumericReasoner.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `QueryServiceTests.swift` covering prompt #88's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/NumericReasoner.swift` + `NotchReducer.swift`
