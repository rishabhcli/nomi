# [A-145] Grounding: Confidence.swift citation integrity
...
Raise retrieval/citation quality in `Confidence.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `NotchReducerTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/Confidence.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `NotchReducerTests.swift`
