# [A-206] Memory: Highlight.swift dynamics
...
Strengthen memory dynamics in `Highlight.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `StateMachineTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
