# [A-325] Latency: ContextAssembler.swift scheduling
...
Concurrency/latency in `ContextAssembler.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/ContextAssembler.swift` + `WorkScheduler.swift`
