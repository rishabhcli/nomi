# [A-190] Ingestion: QueryHistory.swift reliability
...
Improve ingestion/sync in `QueryHistory.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `EgressHostAndCitationTests.swift`.
