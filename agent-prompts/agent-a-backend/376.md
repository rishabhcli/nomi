# [A-376] CLI: expose LLMHopPlanner.swift via mnemoctl
...
Wire `LLMHopPlanner.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `LLMHopPlanner.swift`
