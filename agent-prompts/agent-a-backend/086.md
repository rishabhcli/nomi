# [A-086] Query lifecycle: QueryHistory.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 86 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `QueryHistory.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `CommandParserTests.swift` covering prompt #86's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryHistory.swift` + `NotchReducer.swift`
