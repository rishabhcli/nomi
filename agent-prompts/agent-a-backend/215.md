# [A-215] Memory: CitationVerifier.swift dynamics
...
Strengthen memory dynamics in `CitationVerifier.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `MemoryDynamicsTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `MemoryDynamicsTests.swift`
