# [A-159] Grounding: RouterEscalator.swift citation integrity
...
Raise retrieval/citation quality in `RouterEscalator.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `InspectorTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/RouterEscalator.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `InspectorTests.swift`
