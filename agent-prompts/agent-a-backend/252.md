# [A-252] Consolidation: EntityExtractor.swift dreaming safety
...
Consolidation pass in `EntityExtractor.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `SmarterThanSiriTests.swift`.
