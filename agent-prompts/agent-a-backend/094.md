# [A-094] Query lifecycle: Provenance.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 94 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `Provenance.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `EngineIntegrationTests.swift` covering prompt #94's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/Provenance.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/EngineIntegrationTests.swift` or `QueryLifecycleTests.swift`
