# [A-162] Ingestion: EngineIntegration.swift reliability
...
Improve ingestion/sync in `EngineIntegration.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `SchedulerTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `SchedulerTests.swift`
