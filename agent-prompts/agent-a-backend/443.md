# [A-443] Beats-Siri: WorkScheduler.swift offline synthesis
...
Beats-Siri gate for `WorkScheduler.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #443.
