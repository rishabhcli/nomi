# [A-344] Latency: AdaptiveEffort.swift scheduling
...
Concurrency/latency in `AdaptiveEffort.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/AdaptiveEffort.swift` + `WorkScheduler.swift`
