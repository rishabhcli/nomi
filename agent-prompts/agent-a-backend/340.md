# [A-340] Latency: NotchReducer.swift scheduling
...
Concurrency/latency in `NotchReducer.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/NotchReducer.swift` + `WorkScheduler.swift`
