# [A-132] Grounding: NotchReducer.swift citation integrity

| Field | Value |
|-------|-------|
| **Queue position** | 132 of 500 (chronological — do not skip) |
| **Agent** | A — Backend / Orchestrator |
| **Phase** | Retrieval & Citations — Engine client, agentic grep, verifier, spans |
| **Priority** | P1 |
...
Raise retrieval/citation quality in `NotchReducer.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `EgressHostAndCitationTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `EgressHostAndCitationTests.swift`
