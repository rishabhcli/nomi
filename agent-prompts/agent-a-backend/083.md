# [A-083] Query lifecycle: ScopeClassifier.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 83 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `ScopeClassifier.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `CharSpanTests.swift` covering prompt #83's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/ScopeClassifier.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CharSpanTests.swift` or `QueryLifecycleTests.swift`
