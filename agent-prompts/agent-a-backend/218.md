# [A-218] Memory: AgenticGrep.swift dynamics
...
Strengthen memory dynamics in `AgenticGrep.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `EgressGuardTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `EgressGuardTests.swift`
