# [A-232] Memory: Inspector.swift dynamics
...
Strengthen memory dynamics in `Inspector.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `NotchReducerTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `NotchReducerTests.swift`
