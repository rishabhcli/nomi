# [A-204] Memory: Preferences.swift dynamics
...
Strengthen memory dynamics in `Preferences.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `QueryServiceTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `QueryServiceTests.swift`
