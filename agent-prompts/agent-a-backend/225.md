# [A-225] Memory: IngestGate.swift dynamics
...
Strengthen memory dynamics in `IngestGate.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `ExpressivenessTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `ExpressivenessTests.swift`
