# [A-360] Latency: Preferences.swift scheduling
...
Concurrency/latency in `Preferences.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/Preferences.swift` + `WorkScheduler.swift`
