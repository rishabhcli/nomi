# [A-271] Consolidation: KeywordBackstop.swift dreaming safety
...
Consolidation pass in `KeywordBackstop.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `IngestGateTests.swift`.
