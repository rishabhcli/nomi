# [A-155] Grounding: ActionExtractor.swift citation integrity
...
Raise retrieval/citation quality in `ActionExtractor.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `IngestGateTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/ActionExtractor.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `IngestGateTests.swift`
