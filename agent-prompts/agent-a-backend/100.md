# [A-100] Query lifecycle: Preferences.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 100 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `Preferences.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `ConsolidationTests.swift` covering prompt #100's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/Preferences.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/ConsolidationTests.swift` or `QueryLifecycleTests.swift`
