# [A-001] Foundation audit: QueryService.swift
...
Read `QueryService.swift` end-to-end and document every public entry point in a code comment block at file top (max 15 lines). Add one missing unit test in `QueryServiceTests.swift` for an uncovered branch.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/QueryServiceTests.swift`
