# [A-323] Latency: KeywordBackstop.swift scheduling
...
Concurrency/latency in `KeywordBackstop.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/KeywordBackstop.swift` + `WorkScheduler.swift`
