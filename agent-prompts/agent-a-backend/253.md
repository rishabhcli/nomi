# [A-253] Consolidation: MediaCompanion.swift dreaming safety
...
Consolidation pass in `MediaCompanion.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `IntelligenceTests.swift`.
