# [A-239] Memory: ScopeClassifier.swift dynamics
...
Strengthen memory dynamics in `ScopeClassifier.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `EngineIntegrationTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `EngineIntegrationTests.swift`
