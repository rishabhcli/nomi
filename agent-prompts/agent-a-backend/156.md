# [A-156] Grounding: CorpusSuggester.swift citation integrity
...
Raise retrieval/citation quality in `CorpusSuggester.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `SyncEngineTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/CorpusSuggester.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `SyncEngineTests.swift`
