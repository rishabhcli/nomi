# [A-331] Latency: ContentHash.swift scheduling
...
Concurrency/latency in `ContentHash.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/ContentHash.swift` + `WorkScheduler.swift`
