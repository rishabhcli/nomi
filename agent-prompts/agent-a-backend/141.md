# [A-141] Grounding: TimeWindow.swift citation integrity
...
Raise retrieval/citation quality in `TimeWindow.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `CharSpanTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `CharSpanTests.swift`
