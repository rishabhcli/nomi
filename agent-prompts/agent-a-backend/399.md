# [A-399] CLI: expose PersonalRanker.swift via mnemoctl
...
Wire `PersonalRanker.swift` behavior through `mnemoctl` for headless acceptance: human-readable progress, JSON `--format` option, exit codes documented. Mirror AT-M* naming.
...
**Primary targets:** `Sources/mnemoctl/main.swift` + `PersonalRanker.swift`
