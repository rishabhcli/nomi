# [A-219] Memory: KeywordBackstop.swift dynamics

| Field | Value |
|-------|-------|
| **Queue position** | 219 of 500 (chronological — do not skip) |
| **Agent** | A — Backend / Orchestrator |
| **Phase** | Memory Dynamics — Supersession, TTL, contradiction, inspector ledger |
| **Priority** | P2 |
...
Strengthen memory dynamics in `KeywordBackstop.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `EgressHostAndCitationTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
**Primary targets:** `Sources/MnemoOrchestrator/KeywordBackstop.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `EgressHostAndCitationTests.swift`
