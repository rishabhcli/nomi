# [A-257] Consolidation: Coverage.swift dreaming safety
...
Consolidation pass in `Coverage.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `CharSpanTests.swift`.
