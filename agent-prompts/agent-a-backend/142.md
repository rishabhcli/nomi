# [A-142] Grounding: TimelineBuilder.swift citation integrity
...
Raise retrieval/citation quality in `TimelineBuilder.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `DocumentsTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/TimelineBuilder.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `DocumentsTests.swift`
