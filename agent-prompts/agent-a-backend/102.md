# [A-102] Query lifecycle: Highlight.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 102 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `Highlight.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `EgressGuardTests.swift` covering prompt #102's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/Highlight.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/EgressGuardTests.swift` or `QueryLifecycleTests.swift`
