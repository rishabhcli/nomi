# [A-172] Ingestion: Ingestion.swift reliability
...
Improve ingestion/sync in `Ingestion.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `ProfileTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `ProfileTests.swift`
