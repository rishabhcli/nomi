# [A-222] Memory: Prompt.swift dynamics
...
Strengthen memory dynamics in `Prompt.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `HelpfulnessTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
**Primary targets:** `Sources/MnemoOrchestrator/Prompt.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `HelpfulnessTests.swift`
