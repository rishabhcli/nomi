# [A-157] Grounding: QueryService.swift citation integrity
...
Raise retrieval/citation quality in `QueryService.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `MemoryDynamicsTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryService.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `MemoryDynamicsTests.swift`
