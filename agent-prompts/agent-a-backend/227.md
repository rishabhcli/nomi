# [A-227] Memory: ContentHash.swift dynamics
...
Strengthen memory dynamics in `ContentHash.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `SpanResolverTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `SpanResolverTests.swift`
