# [A-258] Consolidation: Highlight.swift dreaming safety
...
Consolidation pass in `Highlight.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `DocumentsTests.swift`.
