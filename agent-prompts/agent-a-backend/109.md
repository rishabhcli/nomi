# [A-109] Query lifecycle: EngineClient.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 109 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `EngineClient.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `ExpressivenessTests.swift` covering prompt #109's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/EngineClient.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/ExpressivenessTests.swift` or `QueryLifecycleTests.swift`
