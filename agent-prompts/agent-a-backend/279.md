# [A-279] Consolidation: ContentHash.swift dreaming safety
...
Consolidation pass in `ContentHash.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `BugfixRegressionTests.swift`.
