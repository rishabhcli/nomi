# [A-085] Query lifecycle: AnswerCache.swift event completeness
...
Harden query lifecycle in `AnswerCache.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `ProfileTests.swift` covering prompt #85's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/AnswerCache.swift` + `NotchReducer.swift`
