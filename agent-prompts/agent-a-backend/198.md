# [A-198] Ingestion: Provenance.swift reliability
...
Improve ingestion/sync in `Provenance.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `SpanResolverTests.swift`.
