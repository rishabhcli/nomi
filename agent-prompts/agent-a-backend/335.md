# [A-335] Latency: LLMSynthesizer.swift scheduling
...
Concurrency/latency in `LLMSynthesizer.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/LLMSynthesizer.swift` + `WorkScheduler.swift`
