# [A-087] Query lifecycle: PersonalRanker.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 87 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `PersonalRanker.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `NotchReducerTests.swift` covering prompt #87's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/PersonalRanker.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/NotchReducerTests.swift` or `QueryLifecycleTests.swift`
