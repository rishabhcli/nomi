# [A-146] Grounding: Provenance.swift citation integrity
...
Raise retrieval/citation quality in `Provenance.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `QueryServiceTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/Provenance.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `QueryServiceTests.swift`
