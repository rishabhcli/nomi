# [A-205] Memory: Coverage.swift dynamics
...
Strengthen memory dynamics in `Coverage.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `QueryLifecycleTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
