# [A-278] Consolidation: SyncEngine.swift dreaming safety
...
Consolidation pass in `SyncEngine.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `SchedulerTests.swift`.
