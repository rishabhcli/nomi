# [A-237] Memory: QueryRewriter.swift dynamics
...
Strengthen memory dynamics in `QueryRewriter.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `CitationVerifierTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
