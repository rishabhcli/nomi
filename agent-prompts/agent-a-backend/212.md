# [A-212] Memory: EvidenceGathering.swift dynamics
...
Strengthen memory dynamics in `EvidenceGathering.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `IngestIndexTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `IngestIndexTests.swift`
