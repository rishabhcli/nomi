# [A-197] Ingestion: Confidence.swift reliability
...
Improve ingestion/sync in `Confidence.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `HopPlannerTests.swift`.
