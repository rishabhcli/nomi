# [A-164] Ingestion: SpanResolver.swift reliability
...
Improve ingestion/sync in `SpanResolver.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `HelpfulnessTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `HelpfulnessTests.swift`
