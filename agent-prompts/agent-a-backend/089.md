# [A-089] Query lifecycle: TimeWindow.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 89 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `TimeWindow.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `QueryLifecycleTests.swift` covering prompt #89's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/TimeWindow.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/QueryLifecycleTests.swift` or `QueryLifecycleTests.swift`
