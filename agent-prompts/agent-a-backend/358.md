# [A-358] Latency: LocalExtractor.swift scheduling
...
Concurrency/latency in `LocalExtractor.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/LocalExtractor.swift` + `WorkScheduler.swift`
