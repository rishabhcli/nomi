# [A-167] Ingestion: KeywordBackstop.swift reliability
...
Improve ingestion/sync in `KeywordBackstop.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `ExpressivenessTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `ExpressivenessTests.swift`
