# [A-238] Memory: QueryDecomposer.swift dynamics
...
Strengthen memory dynamics in `QueryDecomposer.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `EngineClientTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `EngineClientTests.swift`
