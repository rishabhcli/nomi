# [A-463] Beats-Siri: Digest.swift offline synthesis
...
Beats-Siri gate for `Digest.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #463.
