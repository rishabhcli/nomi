# [A-129] Grounding: Profile.swift citation integrity
...
Raise retrieval/citation quality in `Profile.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `ConsolidationTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/Profile.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `ConsolidationTests.swift`
