# [A-356] Latency: EntityExtractor.swift scheduling
...
Concurrency/latency in `EntityExtractor.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/EntityExtractor.swift` + `WorkScheduler.swift`
