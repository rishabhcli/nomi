# [A-115] Query lifecycle: KeywordBackstop.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 115 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `KeywordBackstop.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `CommandParserTests.swift` covering prompt #115's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/KeywordBackstop.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CommandParserTests.swift` or `QueryLifecycleTests.swift`
