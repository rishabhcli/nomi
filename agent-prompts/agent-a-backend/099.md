# [A-099] Query lifecycle: Digest.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 99 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `Digest.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `MemoryDynamicsTests.swift` covering prompt #99's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/Digest.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `QueryLifecycleTests.swift`
