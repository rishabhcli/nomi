# [A-168] Ingestion: LLMHopPlanner.swift reliability
...
Improve ingestion/sync in `LLMHopPlanner.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `HopPlannerTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/LLMHopPlanner.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `HopPlannerTests.swift`
