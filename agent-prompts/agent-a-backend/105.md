# [A-105] Query lifecycle: QueryService.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 105 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `QueryService.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `BugfixRegressionTests.swift` covering prompt #105's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/QueryService.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/BugfixRegressionTests.swift` or `QueryLifecycleTests.swift`
