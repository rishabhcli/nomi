# [A-136] Grounding: AdaptiveEffort.swift citation integrity
...
Raise retrieval/citation quality in `AdaptiveEffort.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `SmarterThanSiriTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/AdaptiveEffort.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `SmarterThanSiriTests.swift`
