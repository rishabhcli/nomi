# [A-210] Memory: Router.swift dynamics
...
Strengthen memory dynamics in `Router.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `EngineIntegrationTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
