# [A-183] Ingestion: WorkScheduler.swift reliability
...
Improve ingestion/sync in `WorkScheduler.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `IngestIndexTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `IngestIndexTests.swift`
