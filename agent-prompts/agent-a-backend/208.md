# [A-208] Memory: CorpusSuggester.swift dynamics
...
Strengthen memory dynamics in `CorpusSuggester.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `CitationVerifierTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `CitationVerifierTests.swift`
