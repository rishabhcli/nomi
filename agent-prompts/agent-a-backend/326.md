# [A-326] Latency: Prompt.swift scheduling
...
Concurrency/latency in `Prompt.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/Prompt.swift` + `WorkScheduler.swift`
