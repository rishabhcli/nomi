# [A-355] Latency: CommandParser.swift scheduling
...
Concurrency/latency in `CommandParser.swift`: interactive queries preempt `.utility` background work; measure first-token contribution. Extend `SchedulerTests` / `m11-slo-report.txt` fixture.
...
**Primary targets:** `Sources/MnemoOrchestrator/CommandParser.swift` + `WorkScheduler.swift`
