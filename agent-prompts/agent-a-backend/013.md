# [A-013] Foundation audit: ContextAssembler.swift
...
Verify `ContextAssembler.swift` never constructs a URL with a non-loopback host. Add invariant assertion test in `InvariantTests` or `ConsolidationTests.swift`.
