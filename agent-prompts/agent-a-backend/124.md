# [A-124] Grounding: MemoryDynamics.swift citation integrity
...
Raise retrieval/citation quality in `MemoryDynamics.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `AgenticGrepTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/MemoryDynamics.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `AgenticGrepTests.swift`
