# [A-261] Consolidation: QueryService.swift dreaming safety
...
Consolidation pass in `QueryService.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `NotchReducerTests.swift`.
