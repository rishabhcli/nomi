# [A-211] Memory: RouterEscalator.swift dynamics
...
Strengthen memory dynamics in `RouterEscalator.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `AgenticGrepTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `AgenticGrepTests.swift`
