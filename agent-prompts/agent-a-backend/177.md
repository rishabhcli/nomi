# [A-177] Ingestion: ConflictDetector.swift reliability
...
Improve ingestion/sync in `ConflictDetector.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `StateMachineTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `StateMachineTests.swift`
