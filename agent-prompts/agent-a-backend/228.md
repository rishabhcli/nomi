# [A-228] Memory: MemoryDynamics.swift dynamics
...
Strengthen memory dynamics in `MemoryDynamics.swift`: supersession in-place, TTL expiry, contradiction detection. Prove with `CharSpanTests.swift` that re-asked queries reflect forgotten facts after `/forget`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/MemoryDynamicsTests.swift` or `CharSpanTests.swift`
