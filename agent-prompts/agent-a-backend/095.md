# [A-095] Query lifecycle: CommandParser.swift event completeness

| Field | Value |
|-------|-------|
| **Queue position** | 95 of 500 (chronological — do not skip) |
...
Harden query lifecycle in `CommandParser.swift`: every failure mode must emit a `QueryEvent` that `NotchReducer` can render — never swallow errors. Add lifecycle test in `AgenticGrepTests.swift` covering prompt #95's specific branch (route ambiguity, empty evidence, or retry).
...
**Primary targets:** `Sources/MnemoOrchestrator/CommandParser.swift` + `NotchReducer.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/AgenticGrepTests.swift` or `QueryLifecycleTests.swift`
