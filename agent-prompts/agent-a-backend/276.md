# [A-276] Consolidation: Ingestion.swift dreaming safety
...
Consolidation pass in `Ingestion.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `EgressGuardTests.swift`.
