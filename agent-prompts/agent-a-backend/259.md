# [A-259] Consolidation: ActionExtractor.swift dreaming safety
...
Consolidation pass in `ActionExtractor.swift`: dreaming must not duplicate syntheses, must cite constituents, and must remain subject to M5 verification. Regression in `ProfileTests.swift`.
