# [A-175] Ingestion: ContentHash.swift reliability
...
Improve ingestion/sync in `ContentHash.swift`: indexing terminal state must include accurate path/progress; self-heal must never delete user memories. Test with `Tests/Fixtures/corpus/` in `QueryServiceTests.swift`.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/IngestIndexTests.swift` or `QueryServiceTests.swift`
