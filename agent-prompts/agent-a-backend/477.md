# [A-477] Beats-Siri: CharSpan.swift offline synthesis
...
Beats-Siri gate for `CharSpan.swift`: cross-document offline synthesis with verified citations and zero egress. Capture evidence via `mnemoctl ask --verify` transcript for prompt #477.
