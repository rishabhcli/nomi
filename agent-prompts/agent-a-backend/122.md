# [A-122] Grounding: SyncEngine.swift citation integrity
...
Raise retrieval/citation quality in `SyncEngine.swift`: char-offset spans must resolve to real corpus text; unsupported sentences must reach `.unsupportedAnswer`. Add deterministic fixture test in `EngineClientTests.swift`.
...
**Primary targets:** `Sources/MnemoOrchestrator/SyncEngine.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/CitationVerifierTests.swift` or `EngineClientTests.swift`
