...
| Field | Value |
|-------|-------|
| **Queue position** | 348 of 500 (chronological — do not skip) |
...
Recovery UX in `NotchShape.swift`: one-tap actions for every `TerminalState` recovery enum (waitAndRetry, broaden, addFiles, loadModel, restartEngine) — wired to `AppCommandHandler`.
...
**Primary targets:** `Sources/MnemoApp/AppCommandHandler.swift` + `NotchShape.swift`
