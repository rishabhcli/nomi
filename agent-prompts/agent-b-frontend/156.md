...
Motion spec in `Narrator.swift`: springs summon=0.36/0.84, grow=0.32/0.88, collapse=0.30/0.90; blur-morph on content swap; one `.animation` on `SurfaceGeometry` — eliminate glitch double-animation.
...
**Primary targets:** `Sources/MnemoApp/Motion.swift` + `Narrator.swift`
