...
Liquid Glass fidelity in `Motion.swift`: single `GlassEffectContainer`, `glassEffectID` morph from notch collar to input tray, `.buttonStyle(.glass)`. Glass must not sample glass.
...
**Primary targets:** `Sources/MnemoApp/Motion.swift`
