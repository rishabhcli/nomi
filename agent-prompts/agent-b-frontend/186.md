...
| Field | Value |
|-------|-------|
| **Queue position** | 186 of 500 (chronological — do not skip) |
...
Notch geometry in `NotchShape.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/NotchShape.swift` + `NotchGeometry+NSScreen.swift`
