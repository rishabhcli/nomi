...
| Field | Value |
|-------|-------|
| **Queue position** | 126 of 500 (chronological — do not skip) |
| **Agent** | B — Frontend / Notch UI |
| **Phase** | Motion & Animation — Springs, blur-morph, Reduce Motion, glitch elimination |
| **Priority** | P1 |
...
Motion spec in `NotchGeometry+NSScreen.swift`: springs summon=0.36/0.84, grow=0.32/0.88, collapse=0.30/0.90; blur-morph on content swap; one `.animation` on `SurfaceGeometry` — eliminate glitch double-animation.
...
**Primary targets:** `Sources/MnemoApp/Motion.swift` + `NotchGeometry+NSScreen.swift`
...
- [ ] New/changed behavior has automated test coverage: `scripts/analyze-frames.py` on torture capture
