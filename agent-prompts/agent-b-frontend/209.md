...
| Field | Value |
|-------|-------|
| **Queue position** | 209 of 500 (chronological — do not skip) |
...
Voice pipeline in `VoiceOrb.metal`: on-device `SpeechAnalyzer` only; Metal orb at 120fps; `MicEnvelope` attack/release smooth; press-hold affordance obvious; audio never egresses.
...
**Primary targets:** `Sources/MnemoApp/VoiceOrb.metal` + `VoiceOrb.metal`
