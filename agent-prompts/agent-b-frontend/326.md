...
| Field | Value |
|-------|-------|
| **Queue position** | 326 of 500 (chronological — do not skip) |
...
Recovery UX in `SurfaceBlocks.swift`: one-tap actions for every `TerminalState` recovery enum (waitAndRetry, broaden, addFiles, loadModel, restartEngine) — wired to `AppCommandHandler`.
...
**Primary targets:** `Sources/MnemoApp/AppCommandHandler.swift` + `SurfaceBlocks.swift`
