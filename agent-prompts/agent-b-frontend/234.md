...
| Field | Value |
|-------|-------|
| **Queue position** | 234 of 500 (chronological — do not skip) |
| **Agent** | B — Frontend / Notch UI |
| **Phase** | Voice Orb & Dictation — Metal shader 120fps, mic envelope, on-device Speech |
| **Priority** | P2 |
...
Voice pipeline in `NotchGeometry+NSScreen.swift`: on-device `SpeechAnalyzer` only; Metal orb at 120fps; `MicEnvelope` attack/release smooth; press-hold affordance obvious; audio never egresses.
...
**Primary targets:** `Sources/MnemoApp/NotchGeometry+NSScreen.swift` + `VoiceOrb.metal`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/VoiceOrbTests.swift`
