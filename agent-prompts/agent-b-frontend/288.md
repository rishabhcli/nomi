...
| Field | Value |
|-------|-------|
| **Queue position** | 288 of 500 (chronological — do not skip) |
| **Agent** | B — Frontend / Notch UI |
| **Phase** | Source Cards & Citations UI — Sub-second source render, char-span preview, Finder open |
| **Priority** | P2 |
...
Source cards in `NotchGeometry+NSScreen.swift`: render within `sources_render_ms` SLA; char-span preview on hover; Finder reveal on click; citation chips inline in markdown answer.
...
**Primary targets:** `Sources/MnemoApp/SurfaceBlocks.swift`
...
- [ ] New/changed behavior has automated test coverage: Time-to-first-source-card < 1000ms measured in UI test hook
