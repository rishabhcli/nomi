...
| Field | Value |
|-------|-------|
| **Queue position** | 179 of 500 (chronological — do not skip) |
...
Notch geometry in `main.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/main.swift` + `NotchGeometry+NSScreen.swift`
