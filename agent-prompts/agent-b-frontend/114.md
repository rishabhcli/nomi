...
Liquid Glass fidelity in `NotchShape.swift`: single `GlassEffectContainer`, `glassEffectID` morph from notch collar to input tray, `.buttonStyle(.glass)`. Glass must not sample glass.
...
**Primary targets:** `Sources/MnemoApp/NotchShape.swift`
