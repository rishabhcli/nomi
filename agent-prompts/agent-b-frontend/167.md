...
| Field | Value |
|-------|-------|
| **Queue position** | 167 of 500 (chronological — do not skip) |
...
Notch geometry in `NotchPanel.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/NotchPanel.swift` + `NotchGeometry+NSScreen.swift`
