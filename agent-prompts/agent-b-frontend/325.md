...
| Field | Value |
|-------|-------|
| **Queue position** | 325 of 500 (chronological — do not skip) |
...
Recovery UX in `NotchSurfaceView.swift`: one-tap actions for every `TerminalState` recovery enum (waitAndRetry, broaden, addFiles, loadModel, restartEngine) — wired to `AppCommandHandler`.
...
**Primary targets:** `Sources/MnemoApp/AppCommandHandler.swift` + `NotchSurfaceView.swift`
