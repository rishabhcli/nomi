...
| Field | Value |
|-------|-------|
| **Queue position** | 357 of 500 (chronological — do not skip) |
...
Recovery UX in `CorpusControl.swift`: one-tap actions for every `TerminalState` recovery enum (waitAndRetry, broaden, addFiles, loadModel, restartEngine) — wired to `AppCommandHandler`.
...
**Primary targets:** `Sources/MnemoApp/AppCommandHandler.swift` + `CorpusControl.swift`
