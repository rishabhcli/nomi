...
| Field | Value |
|-------|-------|
| **Queue position** | 235 of 500 (chronological — do not skip) |
...
Voice pipeline in `NotchSurfaceView.swift`: on-device `SpeechAnalyzer` only; Metal orb at 120fps; `MicEnvelope` attack/release smooth; press-hold affordance obvious; audio never egresses.
...
**Primary targets:** `Sources/MnemoApp/NotchSurfaceView.swift` + `VoiceOrb.metal`
