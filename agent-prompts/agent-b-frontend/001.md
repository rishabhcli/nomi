# [B-001] Foundation: NotchSurfaceView.swift architecture audit

| Field | Value |
|-------|-------|
| **Queue position** | 1 of 500 (chronological — do not skip) |
...
Map every `@State` / `@Published` in `NotchSurfaceView.swift` to a `NotchReducer` phase — eliminate orphan UI state that can desync from backend events.
