...
| Field | Value |
|-------|-------|
| **Queue position** | 60 of 500 (chronological — do not skip) |
...
Render terminal state `.empty` in `SurfaceBlocks.swift` with a beautiful, calm layout: iconography, recovery CTA, and plain-language copy. Must match Liquid Glass aesthetic. No empty panels — every sub-case of `.empty` gets distinct copy.
