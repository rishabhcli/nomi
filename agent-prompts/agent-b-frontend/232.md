...
| Field | Value |
|-------|-------|
| **Queue position** | 232 of 500 (chronological — do not skip) |
...
Voice pipeline in `DebugHooks.swift`: on-device `SpeechAnalyzer` only; Metal orb at 120fps; `MicEnvelope` attack/release smooth; press-hold affordance obvious; audio never egresses.
...
**Primary targets:** `Sources/MnemoApp/DebugHooks.swift` + `VoiceOrb.metal`
