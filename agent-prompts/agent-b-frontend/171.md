...
| Field | Value |
|-------|-------|
| **Queue position** | 171 of 500 (chronological — do not skip) |
...
Notch geometry in `Dictation.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/Dictation.swift` + `NotchGeometry+NSScreen.swift`
