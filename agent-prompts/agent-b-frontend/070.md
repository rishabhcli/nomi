...
| Field | Value |
|-------|-------|
| **Queue position** | 70 of 500 (chronological — do not skip) |
| **Agent** | B — Frontend / Notch UI |
| **Phase** | State Machine Rendering — Every TerminalState and QueryEvent phase renders beautifully |
| **Priority** | P1 |
...
Render terminal state `.unsupportedAnswer` in `SurfaceBlocks.swift` with a beautiful, calm layout: iconography, recovery CTA, and plain-language copy. Must match Liquid Glass aesthetic. No empty panels — every sub-case of `.unsupportedAnswer` gets distinct copy.
...
**Primary targets:** `Sources/MnemoApp/SurfaceBlocks.swift` + `NotchReducer.swift` messages
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/StateMachineTests.swift` + screenshot via `DebugHooks`
