...
| Field | Value |
|-------|-------|
| **Queue position** | 51 of 500 (chronological — do not skip) |
...
Render terminal state `.engineUnreachable` in `SurfaceBlocks.swift` with a beautiful, calm layout: iconography, recovery CTA, and plain-language copy. Must match Liquid Glass aesthetic. No empty panels — every sub-case of `.engineUnreachable` gets distinct copy.
