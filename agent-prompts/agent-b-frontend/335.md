...
| Field | Value |
|-------|-------|
| **Queue position** | 335 of 500 (chronological — do not skip) |
...
Recovery UX in `VoiceOrb.metal`: one-tap actions for every `TerminalState` recovery enum (waitAndRetry, broaden, addFiles, loadModel, restartEngine) — wired to `AppCommandHandler`.
