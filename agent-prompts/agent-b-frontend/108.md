...
| **Phase** | Liquid Glass — Real glassEffect APIs, single container, morph IDs |
| **Priority** | P1 |
...
Liquid Glass fidelity in `NotchGeometry+NSScreen.swift`: single `GlassEffectContainer`, `glassEffectID` morph from notch collar to input tray, `.buttonStyle(.glass)`. Glass must not sample glass.
...
**Primary targets:** `Sources/MnemoApp/NotchGeometry+NSScreen.swift`
...
- [ ] New/changed behavior has automated test coverage: Visual check + `scripts/ui-torture.sh` frame analysis
