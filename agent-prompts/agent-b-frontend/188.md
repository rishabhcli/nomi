...
| Field | Value |
|-------|-------|
| **Queue position** | 188 of 500 (chronological — do not skip) |
...
Notch geometry in `HoverDetector.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/HoverDetector.swift` + `NotchGeometry+NSScreen.swift`
