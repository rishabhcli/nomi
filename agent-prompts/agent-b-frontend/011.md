...
| Field | Value |
|-------|-------|
| **Queue position** | 11 of 500 (chronological — do not skip) |
...
Map every `@State` / `@Published` in `VoiceOrb.metal` to a `NotchReducer` phase — eliminate orphan UI state that can desync from backend events.
