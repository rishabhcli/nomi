...
| Field | Value |
|-------|-------|
| **Queue position** | 336 of 500 (chronological — do not skip) |
...
Recovery UX in `Narrator.swift`: one-tap actions for every `TerminalState` recovery enum (waitAndRetry, broaden, addFiles, loadModel, restartEngine) — wired to `AppCommandHandler`.
...
**Primary targets:** `Sources/MnemoApp/AppCommandHandler.swift` + `Narrator.swift`
