...
| Field | Value |
|-------|-------|
| **Queue position** | 414 of 500 (chronological — do not skip) |
| **Agent** | B — Frontend / Notch UI |
| **Phase** | Interaction Polish — Hover dwell, hotkey summon, in-flight query lock |
| **Priority** | P3 |
...
Interaction polish in `NotchGeometry+NSScreen.swift`: hover dwell before collapse; in-flight query lock prevents duplicate summon; single-instance enforcement; global hotkey summon from config.
...
**Primary targets:** `Sources/MnemoApp/NotchController.swift` + `HoverDetector.swift`
...
- [ ] New/changed behavior has automated test coverage: Regression: rapid hover-out/in during streaming — no duplicate panel
