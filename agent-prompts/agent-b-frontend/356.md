...
| Field | Value |
|-------|-------|
| **Queue position** | 356 of 500 (chronological — do not skip) |
...
Recovery UX in `BackgroundSync.swift`: one-tap actions for every `TerminalState` recovery enum (waitAndRetry, broaden, addFiles, loadModel, restartEngine) — wired to `AppCommandHandler`.
...
**Primary targets:** `Sources/MnemoApp/AppCommandHandler.swift` + `BackgroundSync.swift`
