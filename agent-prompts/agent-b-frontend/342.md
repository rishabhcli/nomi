...
| Field | Value |
|-------|-------|
| **Queue position** | 342 of 500 (chronological — do not skip) |
...
Recovery UX in `NotchGeometry+NSScreen.swift`: one-tap actions for every `TerminalState` recovery enum (waitAndRetry, broaden, addFiles, loadModel, restartEngine) — wired to `AppCommandHandler`.
