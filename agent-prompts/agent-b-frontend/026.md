...
| Field | Value |
|-------|-------|
| **Queue position** | 26 of 500 (chronological — do not skip) |
...
Map every `@State` / `@Published` in `HoverDetector.swift` to a `NotchReducer` phase — eliminate orphan UI state that can desync from backend events.
...
**Primary targets:** `Sources/MnemoApp/HoverDetector.swift`
