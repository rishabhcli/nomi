...
Liquid Glass fidelity in `AppCommandHandler.swift`: single `GlassEffectContainer`, `glassEffectID` morph from notch collar to input tray, `.buttonStyle(.glass)`. Glass must not sample glass.
...
**Primary targets:** `Sources/MnemoApp/AppCommandHandler.swift`
