...
| Field | Value |
|-------|-------|
| **Queue position** | 180 of 500 (chronological — do not skip) |
| **Agent** | B — Frontend / Notch UI |
| **Phase** | Notch Geometry — Physical + virtual notch, multi-display, runtime NSScreen |
| **Priority** | P1 |
...
Notch geometry in `NotchGeometry+NSScreen.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/NotchGeometry+NSScreen.swift` + `NotchGeometry+NSScreen.swift`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/NotchGeometryTests.swift`
