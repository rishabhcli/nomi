...
| Field | Value |
|-------|-------|
| **Queue position** | 347 of 500 (chronological — do not skip) |
...
Recovery UX in `NotchPanel.swift`: one-tap actions for every `TerminalState` recovery enum (waitAndRetry, broaden, addFiles, loadModel, restartEngine) — wired to `AppCommandHandler`.
...
**Primary targets:** `Sources/MnemoApp/AppCommandHandler.swift` + `NotchPanel.swift`
