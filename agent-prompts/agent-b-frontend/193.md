...
| Field | Value |
|-------|-------|
| **Queue position** | 193 of 500 (chronological — do not skip) |
...
Notch geometry in `AppCommandHandler.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/AppCommandHandler.swift` + `NotchGeometry+NSScreen.swift`
