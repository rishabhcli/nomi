...
| Field | Value |
|-------|-------|
| **Queue position** | 50 of 500 (chronological — do not skip) |
...
Render terminal state `.modelNotLoaded` in `SurfaceBlocks.swift` with a beautiful, calm layout: iconography, recovery CTA, and plain-language copy. Must match Liquid Glass aesthetic. No empty panels — every sub-case of `.modelNotLoaded` gets distinct copy.
