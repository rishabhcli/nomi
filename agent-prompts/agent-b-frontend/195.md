...
| Field | Value |
|-------|-------|
| **Queue position** | 195 of 500 (chronological — do not skip) |
...
Notch geometry in `CorpusControl.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/CorpusControl.swift` + `NotchGeometry+NSScreen.swift`
