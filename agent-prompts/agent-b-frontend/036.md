...
| Field | Value |
|-------|-------|
| **Queue position** | 36 of 500 (chronological — do not skip) |
| **Agent** | B — Frontend / Notch UI |
| **Phase** | Foundation — Understand notch architecture, reducer contract, panel lifecycle |
| **Priority** | P0 |
...
Map every `@State` / `@Published` in `NotchGeometry+NSScreen.swift` to a `NotchReducer` phase — eliminate orphan UI state that can desync from backend events.
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/NotchReducerTests.swift` (logic) + manual hover check
