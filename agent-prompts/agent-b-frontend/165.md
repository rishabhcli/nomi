...
| Field | Value |
|-------|-------|
| **Queue position** | 165 of 500 (chronological — do not skip) |
...
Notch geometry in `NotchViewModel.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/NotchViewModel.swift` + `NotchGeometry+NSScreen.swift`
