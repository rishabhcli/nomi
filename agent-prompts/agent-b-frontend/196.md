...
| Field | Value |
|-------|-------|
| **Queue position** | 196 of 500 (chronological — do not skip) |
...
Notch geometry in `DebugHooks.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/DebugHooks.swift` + `NotchGeometry+NSScreen.swift`
