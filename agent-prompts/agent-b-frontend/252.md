...
**Observability UI:** beautifully surface `QueryEvent.done` in the notch answer zone — animated reasoning trace, retrieval hops, verification progress. User must *see* what Mnemo is doing without raw logs. Design: collapsible timeline, subtle glass chips, auto-dismiss on `.done`.
