...
| Field | Value |
|-------|-------|
| **Queue position** | 163 of 500 (chronological — do not skip) |
...
Notch geometry in `NotchSurfaceView.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/NotchSurfaceView.swift` + `NotchGeometry+NSScreen.swift`
