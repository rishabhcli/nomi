...
Liquid Glass fidelity in `NotchController.swift`: single `GlassEffectContainer`, `glassEffectID` morph from notch collar to input tray, `.buttonStyle(.glass)`. Glass must not sample glass.
...
**Primary targets:** `Sources/MnemoApp/NotchController.swift`
