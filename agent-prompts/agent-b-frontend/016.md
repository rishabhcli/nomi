...
| Field | Value |
|-------|-------|
| **Queue position** | 16 of 500 (chronological — do not skip) |
...
Map every `@State` / `@Published` in `DebugHooks.swift` to a `NotchReducer` phase — eliminate orphan UI state that can desync from backend events.
