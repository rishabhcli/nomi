...
| Field | Value |
|-------|-------|
| **Queue position** | 176 of 500 (chronological — do not skip) |
...
Notch geometry in `BackgroundSync.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/BackgroundSync.swift` + `NotchGeometry+NSScreen.swift`
