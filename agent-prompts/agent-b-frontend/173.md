...
| Field | Value |
|-------|-------|
| **Queue position** | 173 of 500 (chronological — do not skip) |
...
Notch geometry in `VoiceOrb.metal`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/VoiceOrb.metal` + `NotchGeometry+NSScreen.swift`
