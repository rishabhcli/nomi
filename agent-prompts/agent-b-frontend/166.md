...
| Field | Value |
|-------|-------|
| **Queue position** | 166 of 500 (chronological — do not skip) |
...
Notch geometry in `NotchController.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/NotchController.swift` + `NotchGeometry+NSScreen.swift`
