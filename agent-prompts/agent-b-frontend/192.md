...
| Field | Value |
|-------|-------|
| **Queue position** | 192 of 500 (chronological — do not skip) |
...
Notch geometry in `Narrator.swift`: read `NSScreen.safeAreaInsets` and `auxiliaryTopLeftArea` at runtime; virtual notch on external displays; multi-monitor hover handoff without duplicate panels.
...
**Primary targets:** `Sources/MnemoApp/Narrator.swift` + `NotchGeometry+NSScreen.swift`
