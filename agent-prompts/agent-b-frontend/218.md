...
| Field | Value |
|-------|-------|
| **Queue position** | 218 of 500 (chronological — do not skip) |
...
Voice pipeline in `SurfaceBlocks.swift`: on-device `SpeechAnalyzer` only; Metal orb at 120fps; `MicEnvelope` attack/release smooth; press-hold affordance obvious; audio never egresses.
...
**Primary targets:** `Sources/MnemoApp/SurfaceBlocks.swift` + `VoiceOrb.metal`
