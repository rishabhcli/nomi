...
| Field | Value |
|-------|-------|
| **Queue position** | 266 of 500 (chronological — do not skip) |
| **Agent** | B — Frontend / Notch UI |
| **Phase** | Reasoning & Observability UI — Surface .reasoning, .understanding, provenance beautifully |
| **Priority** | P2 |
...
**Observability UI:** beautifully surface `QueryEvent.understanding` in the notch answer zone — animated reasoning trace, retrieval hops, verification progress. User must *see* what Mnemo is doing without raw logs. Design: collapsible timeline, subtle glass chips, auto-dismiss on `.done`.
...
**Primary targets:** `Sources/MnemoApp/SurfaceBlocks.swift` + `NotchSurfaceView.swift`
...
- [ ] New/changed behavior has automated test coverage: Manual: submit query, confirm reasoning visible before answer completes
