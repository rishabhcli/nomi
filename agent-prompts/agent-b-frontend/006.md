...
| Field | Value |
|-------|-------|
| **Queue position** | 6 of 500 (chronological — do not skip) |
...
Map every `@State` / `@Published` in `NotchShape.swift` to a `NotchReducer` phase — eliminate orphan UI state that can desync from backend events.
