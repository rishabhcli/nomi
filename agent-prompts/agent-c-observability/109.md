# [C-109] Config: wire router.escalation_threshold
...
Parse and validate `mnemo.toml` key `router.escalation_threshold`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
