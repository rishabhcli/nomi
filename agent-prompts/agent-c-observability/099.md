# [C-099] Config: wire bench.warmup_runs
...
Parse and validate `mnemo.toml` key `bench.warmup_runs`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
