# [C-311] CLI observability: restart-engine command polish

| Field | Value |
|-------|-------|
| **Queue position** | 311 of 500 (chronological — do not skip) |
| **Agent** | C — Observability / Infra |
| **Phase** | mnemoctl Commands — Every subcommand documented, tested, observable output |
| **Priority** | P2 |
...
`mnemoctl restart-engine`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (restart-engine subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl restart-engine --help && offline invocation
