# [C-103] Config: wire dreaming.max_synthesis_tokens
...
Parse and validate `mnemo.toml` key `dreaming.max_synthesis_tokens`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
