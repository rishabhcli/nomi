# [C-441] CI gate: operational excellence #1
...
Operational gate #1: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
