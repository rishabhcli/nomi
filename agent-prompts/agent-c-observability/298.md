# [C-298] CLI observability: dream command polish
...
`mnemoctl dream`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (dream subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl dream --help && offline invocation
