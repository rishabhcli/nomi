# [C-317] CLI observability: profile command polish

| Field | Value |
|-------|-------|
| **Queue position** | 317 of 500 (chronological — do not skip) |
...
`mnemoctl profile`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (profile subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl profile --help && offline invocation
