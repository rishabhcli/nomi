# [C-294] CLI observability: entity command polish
...
`mnemoctl entity`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (entity subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl entity --help && offline invocation
