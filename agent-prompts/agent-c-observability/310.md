# [C-310] CLI observability: stop command polish
...
`mnemoctl stop`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (stop subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl stop --help && offline invocation
