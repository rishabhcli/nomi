# [C-472] CI gate: operational excellence #32

| Field | Value |
|-------|-------|
| **Queue position** | 472 of 500 (chronological — do not skip) |
| **Agent** | C — Observability / Infra |
| **Phase** | Operational Excellence — CI gates, release checklist, observability dashboard hooks |
| **Priority** | P3 |
...
Operational gate #32: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
...
**Primary targets:** `.github/workflows/` or `scripts/ci.sh`
...
- [ ] New/changed behavior has automated test coverage: Simulated CI run exit 0
