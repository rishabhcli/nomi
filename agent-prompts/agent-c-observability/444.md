# [C-444] CI gate: operational excellence #4
...
Operational gate #4: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
