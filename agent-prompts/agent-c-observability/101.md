# [C-101] Config: wire health.probe_interval
...
Parse and validate `mnemo.toml` key `health.probe_interval`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
