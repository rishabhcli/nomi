# [C-306] CLI observability: forget-scope command polish
...
`mnemoctl forget-scope`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (forget-scope subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl forget-scope --help && offline invocation
