# [C-319] CLI observability: media-sync command polish
...
`mnemoctl media-sync`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (media-sync subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl media-sync --help && offline invocation
