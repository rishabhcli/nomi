# [C-315] CLI observability: ingest-status command polish
...
`mnemoctl ingest-status`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (ingest-status subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl ingest-status --help && offline invocation
