# [C-069] Structured log: emit egress_blocked_count
...
Implement structured logging of `egress_blocked_count` to `~/Library/Logs/Mnemo/app.jsonl` (one JSON object per query, no document body at info level). Add redaction tests.
