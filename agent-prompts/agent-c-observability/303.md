# [C-303] CLI observability: processing command polish
...
`mnemoctl processing`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (processing subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl processing --help && offline invocation
