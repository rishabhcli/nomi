# [C-146] Privacy: egress measurement hardening #26

| Field | Value |
|-------|-------|
| **Queue position** | 146 of 500 (chronological — do not skip) |
| **Agent** | C — Observability / Infra |
| **Phase** | Egress & Privacy — Guard hardening, privacy indicator contract, airplane parity |
| **Priority** | P1 |
...
Egress observability #26: strengthen `EgressGuard` measurement; ensure `mnemoctl egress-check` prints blocked host list; `scripts/airplane-parity.sh` step maps to this invariant.
...
**Primary targets:** `Sources/MnemoOrchestrator/EgressGuard.swift` (read-only contract) + `mnemoctl`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoOrchestratorTests/EgressGuardTests.swift` + `airplane-parity.sh`
