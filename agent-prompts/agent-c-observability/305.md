# [C-305] CLI observability: context command polish
...
`mnemoctl context`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (context subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl context --help && offline invocation
