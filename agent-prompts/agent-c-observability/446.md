# [C-446] CI gate: operational excellence #6
...
Operational gate #6: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
