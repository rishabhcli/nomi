# [C-255] Harness: build-app.sh reliability
...
Harness #15: fix `scripts/build-app.sh` — isolated build dir for `run-usecases.sh`, no mid-run binary clobber; green 105-case run; capture log artifact.
...
**Primary targets:** `scripts/build-app.sh`
...
- [ ] New/changed behavior has automated test coverage: Run ./build-app.sh and capture exit 0 + output
