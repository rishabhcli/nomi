# [C-108] Config: wire context.max_tokens
...
Parse and validate `mnemo.toml` key `context.max_tokens`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
