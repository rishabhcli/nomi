# [C-448] CI gate: operational excellence #8
...
Operational gate #8: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
