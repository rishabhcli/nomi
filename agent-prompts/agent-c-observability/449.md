# [C-449] CI gate: operational excellence #9
...
Operational gate #9: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
