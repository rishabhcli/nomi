# [C-296] CLI observability: digest command polish

| Field | Value |
|-------|-------|
| **Queue position** | 296 of 500 (chronological — do not skip) |
...
`mnemoctl digest`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (digest subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl digest --help && offline invocation
