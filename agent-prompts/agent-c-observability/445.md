# [C-445] CI gate: operational excellence #5
...
Operational gate #5: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
