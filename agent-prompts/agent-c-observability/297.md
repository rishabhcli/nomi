# [C-297] CLI observability: sync command polish
...
`mnemoctl sync`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (sync subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl sync --help && offline invocation
