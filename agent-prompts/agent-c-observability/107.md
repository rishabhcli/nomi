# [C-107] Config: wire retrieval.chunk_limit
...
Parse and validate `mnemo.toml` key `retrieval.chunk_limit`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
