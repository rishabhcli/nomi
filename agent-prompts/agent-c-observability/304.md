# [C-304] CLI observability: upload command polish
...
`mnemoctl upload`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (upload subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl upload --help && offline invocation
