# [C-443] CI gate: operational excellence #3
...
Operational gate #3: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
