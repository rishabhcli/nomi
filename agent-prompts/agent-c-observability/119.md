# [C-119] Config: wire smfs.mount_path
...
Parse and validate `mnemo.toml` key `smfs.mount_path`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
