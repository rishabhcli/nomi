# [C-295] CLI observability: preferences command polish
...
`mnemoctl preferences`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (preferences subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl preferences --help && offline invocation
