# [C-302] CLI observability: containers command polish
...
`mnemoctl containers`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (containers subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl containers --help && offline invocation
