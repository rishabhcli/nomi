# [C-318] CLI observability: agentic command polish
...
`mnemoctl agentic`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (agentic subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl agentic --help && offline invocation
