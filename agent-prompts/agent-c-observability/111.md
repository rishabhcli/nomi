# [C-111] Config: wire inspector.suppression_ttl_days

| Field | Value |
|-------|-------|
| **Queue position** | 111 of 500 (chronological — do not skip) |
| **Agent** | C — Observability / Infra |
| **Phase** | Config Wiring — Unparsed mnemo.toml sections → MnemoConfig |
| **Priority** | P1 |
...
Parse and validate `mnemo.toml` key `inspector.suppression_ttl_days`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
...
**Primary targets:** `Sources/MnemoCore/MnemoConfig.swift` + `mnemo.toml`
...
- [ ] New/changed behavior has automated test coverage: `Tests/MnemoCoreTests/ConfigTests.swift`
