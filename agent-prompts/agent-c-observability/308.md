# [C-308] CLI observability: hash command polish
...
`mnemoctl hash`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (hash subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl hash --help && offline invocation
