# [C-110] Config: wire media.retry_count
...
Parse and validate `mnemo.toml` key `media.retry_count`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
