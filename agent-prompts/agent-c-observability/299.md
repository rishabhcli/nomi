# [C-299] CLI observability: inspect command polish
...
`mnemoctl inspect`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (inspect subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl inspect --help && offline invocation
