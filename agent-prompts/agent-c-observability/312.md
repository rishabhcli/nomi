# [C-312] CLI observability: audit command polish
...
`mnemoctl audit`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (audit subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl audit --help && offline invocation
