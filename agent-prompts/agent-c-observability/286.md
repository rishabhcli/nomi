# [C-286] CLI observability: ask command polish
...
`mnemoctl ask`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (ask subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl ask --help && offline invocation
