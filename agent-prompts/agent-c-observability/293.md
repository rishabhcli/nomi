# [C-293] CLI observability: backstop command polish
...
`mnemoctl backstop`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (backstop subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl backstop --help && offline invocation
