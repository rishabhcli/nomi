# [C-105] Config: wire sync.self_heal_enabled
...
Parse and validate `mnemo.toml` key `sync.self_heal_enabled`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
