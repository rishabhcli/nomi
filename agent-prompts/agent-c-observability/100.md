# [C-100] Config: wire bench.sample_size
...
Parse and validate `mnemo.toml` key `bench.sample_size`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
