# [C-115] Config: wire retrieval.max_hops
...
Parse and validate `mnemo.toml` key `retrieval.max_hops`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
