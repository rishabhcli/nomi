# [C-070] Structured log: emit verification_pass_rate

| Field | Value |
|-------|-------|
| **Queue position** | 70 of 500 (chronological — do not skip) |
| **Agent** | C — Observability / Infra |
| **Phase** | Structured Logging — Query-id tracing, latency breakdown, local-only logs |
| **Priority** | P1 |
...
Implement structured logging of `verification_pass_rate` to `~/Library/Logs/Mnemo/app.jsonl` (one JSON object per query, no document body at info level). Add redaction tests.
...
**Primary targets:** New `Sources/MnemoCore/StructuredLog.swift` + wire in `QueryService` via protocol
...
- [ ] New/changed behavior has automated test coverage: Parse log file after `mnemoctl ask`; assert field present
