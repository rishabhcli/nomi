# [C-112] Config: wire engine.timeout_ms
...
Parse and validate `mnemo.toml` key `engine.timeout_ms`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
