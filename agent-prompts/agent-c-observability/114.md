# [C-114] Config: wire ingest.poll_interval
...
Parse and validate `mnemo.toml` key `ingest.poll_interval`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
