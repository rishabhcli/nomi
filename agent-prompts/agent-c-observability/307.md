# [C-307] CLI observability: chunks command polish
...
`mnemoctl chunks`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (chunks subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl chunks --help && offline invocation
