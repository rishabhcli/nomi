# [C-285] CLI observability: health command polish
...
`mnemoctl health`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (health subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl health --help && offline invocation
