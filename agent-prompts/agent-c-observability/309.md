# [C-309] CLI observability: start command polish
...
`mnemoctl start`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (start subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl start --help && offline invocation
