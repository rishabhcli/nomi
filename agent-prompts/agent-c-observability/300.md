# [C-300] CLI observability: egress-check command polish
...
`mnemoctl egress-check`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (egress-check subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl egress-check --help && offline invocation
