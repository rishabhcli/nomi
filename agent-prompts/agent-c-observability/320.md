# [C-320] CLI observability: memory command polish

| Field | Value |
|-------|-------|
| **Queue position** | 320 of 500 (chronological — do not skip) |
...
`mnemoctl memory`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (memory subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl memory --help && offline invocation
