# [C-416] Integration probe: stack health #16

| Field | Value |
|-------|-------|
| **Queue position** | 416 of 500 (chronological — do not skip) |
| **Agent** | C — Observability / Infra |
| **Phase** | Integration Probes — Live stack health, document error rate, extraction failures |
| **Priority** | P3 |
...
Live stack probe #16: script checks engine document error rate, extraction failures, smfs mount health — report JSON for user dashboard.
...
**Primary targets:** `scripts/smoke.sh` extension or new `scripts/stack-report.sh`
...
- [ ] New/changed behavior has automated test coverage: Offline stack returns structured JSON report
