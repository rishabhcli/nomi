# [C-117] Config: wire profile.max_facts
...
Parse and validate `mnemo.toml` key `profile.max_facts`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
