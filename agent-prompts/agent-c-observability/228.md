# [C-228] Metrics: SLA automation #28

| Field | Value |
|-------|-------|
| **Queue position** | 228 of 500 (chronological — do not skip) |
| **Agent** | C — Observability / Infra |
| **Phase** | Metrics & SLA — first_token_ms, sources_render_ms, bench automation |
| **Priority** | P2 |
...
SLA metric #28: automate `mnemoctl bench` threshold comparison against `mnemo.toml [sla]`; emit PASS/FAIL report artifact to `Tests/Fixtures/`.
...
**Primary targets:** `Sources/mnemoctl/main.swift` bench subcommand + `mnemo.toml [sla]`
...
- [ ] New/changed behavior has automated test coverage: Compare output to `Tests/Fixtures/m11-slo-report.txt` format
