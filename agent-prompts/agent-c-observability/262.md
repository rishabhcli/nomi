# [C-262] Harness: drain-watchdog.sh reliability
...
Harness #22: fix `scripts/drain-watchdog.sh` — isolated build dir for `run-usecases.sh`, no mid-run binary clobber; green 105-case run; capture log artifact.
...
**Primary targets:** `scripts/drain-watchdog.sh`
...
- [ ] New/changed behavior has automated test coverage: Run ./drain-watchdog.sh and capture exit 0 + output
