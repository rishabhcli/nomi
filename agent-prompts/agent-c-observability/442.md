# [C-442] CI gate: operational excellence #2
...
Operational gate #2: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
