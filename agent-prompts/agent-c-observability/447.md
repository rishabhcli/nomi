# [C-447] CI gate: operational excellence #7
...
Operational gate #7: CI pipeline step — `swift test` + `mnemoctl audit` + `egress-check` + isolated `run-usecases.sh`; publish badge artifact.
