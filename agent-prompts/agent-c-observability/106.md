# [C-106] Config: wire ingest.max_file_mb
...
Parse and validate `mnemo.toml` key `ingest.max_file_mb`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
