# [C-113] Config: wire model.keepalive
...
Parse and validate `mnemo.toml` key `model.keepalive`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
