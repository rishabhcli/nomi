# [C-098] Config: wire logging.rotation_mb
...
Parse and validate `mnemo.toml` key `logging.rotation_mb`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
