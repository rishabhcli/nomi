# [C-102] Config: wire supervisor.restart_backoff
...
Parse and validate `mnemo.toml` key `supervisor.restart_backoff`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
