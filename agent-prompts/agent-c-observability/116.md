# [C-116] Config: wire verification.strict_mode
...
Parse and validate `mnemo.toml` key `verification.strict_mode`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
