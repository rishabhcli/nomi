# [C-272] Harness: analyze-frames.py reliability
...
Harness #32: fix `scripts/analyze-frames.py` — isolated build dir for `run-usecases.sh`, no mid-run binary clobber; green 105-case run; capture log artifact.
...
**Primary targets:** `scripts/analyze-frames.py`
...
- [ ] New/changed behavior has automated test coverage: Run ./analyze-frames.py and capture exit 0 + output
