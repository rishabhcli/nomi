# [C-251] Harness: run-usecases.sh reliability
...
Harness #11: fix `scripts/run-usecases.sh` — isolated build dir for `run-usecases.sh`, no mid-run binary clobber; green 105-case run; capture log artifact.
...
**Primary targets:** `scripts/run-usecases.sh`
...
- [ ] New/changed behavior has automated test coverage: Run ./run-usecases.sh and capture exit 0 + output
