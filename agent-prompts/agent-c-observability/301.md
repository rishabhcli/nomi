# [C-301] CLI observability: bench command polish
...
`mnemoctl bench`: add `--verbose` reasoning trace for user observability; document flags in `--help`; integration test with stub engine; stable JSON schema.
...
**Primary targets:** `Sources/mnemoctl/main.swift` (bench subcommand)
...
- [ ] New/changed behavior has automated test coverage: .build/debug/mnemoctl bench --help && offline invocation
