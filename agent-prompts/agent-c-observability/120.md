# [C-120] Config: wire logging.level
...
Parse and validate `mnemo.toml` key `logging.level`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
