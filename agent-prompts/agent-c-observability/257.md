# [C-257] Harness: smoke.sh reliability
...
Harness #17: fix `scripts/smoke.sh` — isolated build dir for `run-usecases.sh`, no mid-run binary clobber; green 105-case run; capture log artifact.
...
**Primary targets:** `scripts/smoke.sh`
...
- [ ] New/changed behavior has automated test coverage: Run ./smoke.sh and capture exit 0 + output
