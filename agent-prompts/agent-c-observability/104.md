# [C-104] Config: wire privacy.block_on_egress
...
Parse and validate `mnemo.toml` key `privacy.block_on_egress`: wire into `MnemoConfig`, fail startup if invalid, expose via `mnemoctl health --verbose`.
