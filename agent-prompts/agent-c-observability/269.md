# [C-269] Harness: bulk-ingest.sh reliability
...
Harness #29: fix `scripts/bulk-ingest.sh` — isolated build dir for `run-usecases.sh`, no mid-run binary clobber; green 105-case run; capture log artifact.
...
**Primary targets:** `scripts/bulk-ingest.sh`
...
- [ ] New/changed behavior has automated test coverage: Run ./bulk-ingest.sh and capture exit 0 + output
