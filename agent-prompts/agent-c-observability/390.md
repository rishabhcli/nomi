# [C-390] Debug hooks: headless verification #30

| Field | Value |
|-------|-------|
| **Queue position** | 390 of 500 (chronological — do not skip) |
| **Agent** | C — Observability / Infra |
| **Phase** | Debug & Diagnostics — Debug hooks, geometry logs, headless verification |
| **Priority** | P3 |
...
Diagnostics #30: extend `DebugHooks.swift` contract — geometry snapshots, reasoning event stream to `/tmp/mnemo-geometry.log`, headless PNG captures for CI.
...
**Primary targets:** `Sources/MnemoApp/DebugHooks.swift` (contract only) + infra runner script
...
- [ ] New/changed behavior has automated test coverage: `MNEMO_DEBUG_HOOKS=1` run + log parse
